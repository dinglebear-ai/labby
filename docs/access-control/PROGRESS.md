...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. Implementation includes an unmounted manager composition of coherent runtime Loadout, runtime-pool, and upstream tool-catalog observations. It is not a complete gateway catalog or dispatch grant, and access-control enforcement is not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
...
- [ ] exact Project + policy epoch/complete catalog generation cache key. Wave 16 composes runtime Loadout and upstream-tool facts, but the built-in service registry, subject OAuth catalogs, other MCP capability families, exact targets, and final dispatch boundary remain outside that generation tuple.
