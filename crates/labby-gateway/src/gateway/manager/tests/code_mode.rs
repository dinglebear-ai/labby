...
// ── F0: destructive calls on the decider (pause-capable) path require approval ──
//
// Regression for the "pause never fires" bug: the host used to pass
// `upstream_tool.destructive && !destructive_permitted(surface, caller)` as the
// `requires_approval` argument to `decide()`. On the MCP pause-capable path
// `destructive_permitted(Mcp, caller) == caller.can_execute()`, which is always
// true for any caller able to reach codemode — so the argument was always false
// and the decider never received an approval-required destructive call. The fix
// passes the raw `upstream_tool.destructive` flag on the decider path. This spy
// captures exactly what the host passed and asserts it is `true` for a
// destructive call, and that a `Pause` decision short-circuits the dispatch (the
// upstream stub is never called).
...
/// A minimal `CodeModeDecider` test double. Records the `requires_approval`
/// value each `decide()` call received and returns a scripted `DecideOutcome`.
#[derive(Default)]
struct SpyDecider {
    /// `requires_approval` values captured, in call order.
    captured_requires_approval: Mutex<Vec<bool>>,
    /// Outcome to return from `decide()`.
    outcome: Mutex<Option<DecideOutcome>>,
}

impl SpyDecider {
    fn returning(outcome: DecideOutcome) -> Self {
        Self {
            captured_requires_approval: Mutex::new(Vec::new()),
            outcome: Mutex::new(Some(outcome)),
        }
    }

    fn captured(&self) -> Vec<bool> {
        self.captured_requires_approval.lock().unwrap().clone()
    }
}

impl CodeModeDecider for SpyDecider {
    fn decide<'a>(
        &'a self,
        _execution_id: &'a str,
        _seq: u64,
        ...
        let outcome = self
            ...
            .unwrap()
            .clone()
            .unwrap_or(DecideOutcome::Execute);
    ...
    fn begin(&self, _run: BeginRun) -> BoxDecideFuture<'_, Result<(), ToolError>> {
        Box::pin(async move { Ok(()) })
    }

    fn run_status<'a>(&'a self, _execution_id: &'a str) -> BoxDecideFuture<'a, RunLifecycle> {
        Box::pin(async move { RunLifecycle::Running })
    }

    fn run_error<'a>(&'a self, _execution_id: &'a str) -> BoxDecideFuture<'a, Option<String>> {
    ...
    fn resume_to_running<'a>(&'a self, _execution_id: &'a str) -> BoxDecideFuture<'a, bool> {
...
#[tokio::test]
async fn destructive_call_on_decider_path_requires_approval_and_pauses() {
    let (manager, pool) =
        code_mode_manager_with_upstreams(vec![fixture_http_upstream("alpha")]).await;
    let mut entry = healthy_entry_with_tool("alpha", "delete");
    entry
    ...
    // Spy returns Pause so a destructive fresh call short-circuits before any
    // real upstream dispatch (there is no live HTTP upstream behind the fixture).
    let spy = Arc::new(SpyDecider::returning(DecideOutcome::Pause));
    let manager = manager.with_code_mode_decider(spy.clone());

    // A caller that CAN execute (the only way to reach codemode). Under the old
    // code, `requires_approval` computed by the host would be false for this
    // caller; the fix makes it the raw destructive flag (true).
    ...
    let ctx = labby_codemode::ExecCtx {
        execution_id: Some("exec_test_0"),
        seq: 0,
    };
    ...
    .await
    .expect_err("a Pause decision must return the pause sentinel, not dispatch");

    // (a) The host passed requires_approval=true to decide() for a destructive
    //     call — the crux of the F0 fix.
    assert_eq!(
        spy.captured(),
        vec![true],
        "destructive call on the decider path must pass requires_approval=true"
    );
    // (b) The Pause decision surfaced as the pause sentinel (best-effort halt),
    //     NOT a real upstream dispatch result.
    match err {
        ToolError::Sdk { sdk_kind, .. } => assert_eq!(sdk_kind, "code_mode_paused"),
        other => panic!("expected code_mode_paused sentinel, got {other:?}"),
    }
}

#[tokio::test]
async fn non_destructive_call_on_decider_path_does_not_require_approval() {
    // Symmetric control: a NON-destructive call must pass requires_approval=false
    // so it executes (never pauses). The spy returns Pause here only to prove the
    // captured flag is false for a read tool; it never dispatches upstream.
    let (manager, pool) =
        code_mode_manager_with_upstreams(vec![fixture_http_upstream("alpha")]).await;
    pool.insert_entry_for_tests("alpha", healthy_entry_with_tool("alpha", "ping"))
        .await;
    ...
    .await;
    // The outcome itself is not asserted (the spy returns Pause regardless); the
    // captured flag is the assertion. Bind it so the `#[must_use]` Result is
    ...
    assert_eq!(
        spy.captured(),
        vec![false],
        "non-destructive call must pass requires_approval=false"
    );
}
