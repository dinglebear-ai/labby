...
use super::rows::*;
use super::{SqliteStore, UPSTREAM_OAUTH_STATE_MAX_TTL_SECS, sqlite_error};
...
impl SqliteStore {
    ...
    pub async fn save_upstream_oauth_state(
        ...
        let requested_scopes_json = serde_json::to_string(&row.requested_scopes)
            .map_err(|error| AuthError::Storage(format!("serialize requested scopes: {error}")))?;
        self.with_conn(move |conn| {
            conn.execute(
                "INSERT INTO upstream_oauth_state (
                    upstream_name, subject, csrf_token, pkce_verifier, expected_issuer,
            ...
            conn.query_row(
                "DELETE FROM upstream_oauth_state
                 WHERE upstream_name = ?1
                   ...
                   AND csrf_token = ?3
                   AND expires_at > ?4
                 RETURNING upstream_name, subject, csrf_token, pkce_verifier, expected_issuer,
                           require_issuer, requested_scopes_json, created_at, expires_at",
