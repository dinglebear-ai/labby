...
```bash
git add docs/dev/CODE_MODE_WASMTIME_SPIKE.md crates/labby-codemode/Cargo.toml Cargo.lock deny.toml
git commit -m "docs(codemode): lock wasmtime spike findings"
```
