...
> **Engineering review decision:** `handle_action` is implemented in this slice because it is the correct enforcement point for the destructive confirmation gate (a HIGH security finding — the gate is specified in `api/CLAUDE.md` but was not implemented). ExampleMovies and UniFi HTTP handler migration is **deferred** to a follow-up slice; only ByteStash migrates here.
...
ExampleMovies and UniFi HTTP handler migration is intentionally deferred — they will migrate when those services move to the shared `services/` layer in a follow-up slice.
