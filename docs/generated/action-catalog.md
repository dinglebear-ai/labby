...
| `gateway` | `gateway.code_mode.set` | false | true | `enabled: boolean`<br>`trace_params: boolean`<br>`timeout_ms: integer`<br>`max_tool_calls: integer`<br>`max_response_bytes: integer`<br>`max_response_tokens: integer` | `CodeModeConfig` | cli, mcp, api, web |
