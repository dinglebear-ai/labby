...
| modified (infra, not in this repo) | 10 SWAG proxy confs on squirts (`labby`, `mcp`, `apprise`, `gotify`, `tailscale`, `unraid`, `arcane`, `examplegateway`, `axon`, `synapse`) | Renamed `server_name` from `*.tootie.tv` to `*.dinglebear.ai` | live verification via `curl` per domain |
