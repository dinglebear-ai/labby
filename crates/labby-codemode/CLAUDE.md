...
| Process-group guard | Runner transport spawned with `process_group(0)` (Unix) / Job Object (Windows); `kill_on_drop(true)`; `killpg` reaches descendants. A Microsandbox runner also owns a cleanup guard that force-removes its named microVM on eviction/drop. |
| Env isolation | **Implemented.** Runner transport spawned with `env_clear()` (`pool/runner_handle.rs`, in `PooledRunner::spawn`) — the child inherits NO labby env at all (not even an allowlist), so `LABBY_*` secrets and every other ambient var are excluded. Microsandbox receives configuration as validated argv, never inherited credentials. |
| Optional microVM | **Implemented, opt in on Linux.** Restricted Microsandbox guest; no network; cached image only (`--pull never`); 1 CPU; 256 MiB; bounded duration/idle lifetime; only the validated Labby executable mounted read-only. MCP authority and secrets remain in the host. |
