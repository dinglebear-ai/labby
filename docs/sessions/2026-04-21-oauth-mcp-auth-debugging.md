...
Debug MCP OAuth authentication failures against the lab HTTP server (`https://lab.tootie.tv/mcp`), starting with `{"kind":"validation_failed","message":"resource must be \`https://lab.tootie.tv/mcp\`"}` when trying to auth, and progressing through two subsequent failure modes discovered after each fix.

## Session Overview

Three sequential auth bugs were diagnosed and resolved across two systems (lab server code + SWAG nginx proxy on device `squirts`):

1. **OAuth resource mismatch** — codex client configured with wrong resource URL (`https://lab.tootie.tv` instead of `https://lab.tootie.tv/mcp`). Fixed by user reconfiguring codex.
...
1. User reported `validation_failed` with `resource must be \`https://lab.tootie.tv/mcp\`` during OAuth authorization.
...
4. Checked server logs provided by user: `requested_resource=https://lab.tootie.tv/` vs `expected_resource=https://lab.tootie.tv/mcp` — client sending base URL without `/mcp`.
...
6. User confirmed and fixed: codex OAuth resource config was pointing at base URL; user changed it to `https://lab.tootie.tv/mcp`.
...
9. Read nginx proxy config from `squirts` via SSH: `/mnt/appdata/swag/nginx/proxy-confs/lab.subdomain.conf`.
10. Read `mcp.conf` from squirts: discovered `proxy_set_header Authorization "";` stripping the Bearer token.
...
12. Removed the two offending lines from `/mnt/appdata/swag/nginx/mcp.conf` on squirts via SSH sed.
...
- **`crates/lab/src/api/router.rs:434–443`** — `resource_url` passed to auth middleware (and `WWW-Authenticate` header) is the raw `public_url` base, NOT the canonical MCP resource URL. The WWW-Authenticate header therefore points to `https://lab.tootie.tv/.well-known/oauth-protected-resource`, which correctly returns `resource: https://lab.tootie.tv/mcp` — but only if the client reads the `resource` field.
- **`/mnt/appdata/swag/nginx/mcp.conf:24–25`** (on squirts) — `proxy_set_header Authorization "";` with comment "already validated by /_oauth_verify". The `/_oauth_verify` internal endpoint is not present in the nginx config; this was a half-implemented nginx-level auth pattern that stripped the token without providing the upstream verification gate.
- **Nginx access log on squirts** — confirmed `POST /mcp HTTP/2.0 401 78` with user-agent `claude-code/2.1.116 (cli)` at timestamps matching the lab server's token exchange log entries.
...
|------|----------|--------|
| `/mnt/appdata/swag/nginx/mcp.conf` | device `squirts` | Removed `proxy_set_header Authorization "";` and its orphaned comment |
...
```bash
# Read nginx proxy config from squirts
ssh squirts cat /mnt/appdata/swag/nginx/proxy-confs/lab.subdomain.conf

# Check nginx access and error logs for /mcp entries
ssh squirts "grep -E 'POST /mcp|GET /mcp|DELETE /mcp' /mnt/appdata/swag/log/nginx/access.log | tail -20"
ssh squirts "grep -E '/mcp|mcp' /mnt/appdata/swag/log/nginx/error.log | tail -20"

# Read mcp.conf
ssh squirts "sed -n '1,30p' /mnt/appdata/swag/nginx/mcp.conf"

# Remove Authorization stripping lines
ssh squirts "sed -i '/proxy_set_header Authorization \"\";/d' /mnt/appdata/swag/nginx/mcp.conf"
ssh squirts "sed -i '24d' /mnt/appdata/swag/nginx/mcp.conf"  # remove orphaned comment (em dash broke first sed)

# Verify removal
ssh squirts "grep -n 'Authorization\|oauth_verify' /mnt/appdata/swag/nginx/mcp.conf"
...
- Symptom: `{"kind":"validation_failed","message":"resource must be \`https://lab.tootie.tv/mcp\`"}`
- Root cause: codex MCP client configured with `https://lab.tootie.tv` as the OAuth resource instead of `https://lab.tootie.tv/mcp`
...
- Root cause: `mcp.conf` on squirts nginx contained `proxy_set_header Authorization "";`, stripping the Bearer token before it reached the lab server. Comment referenced a `/_oauth_verify` internal endpoint that does not exist in the config — incomplete nginx-level auth pattern.
...
| Aspect | Before | After |
|--------|--------|-------|
| `codex mcp login lab-http` | Fails with `validation_failed: resource must be \`https://lab.tootie.tv/mcp\`` | Succeeds (user fixed client config) |
...
- **Rollback**: re-add `proxy_set_header Authorization "";` to `/mnt/appdata/swag/nginx/mcp.conf` on squirts and reload nginx. One-liner: `ssh squirts "sed -i '/proxy_set_header Mcp-Session-Id/a proxy_set_header Authorization \"\";' /mnt/appdata/swag/nginx/mcp.conf && docker exec swag nginx -s reload"`

## Decisions Not Taken

- **Server-side resource URL relaxation in `validate_resource()`** — considered accepting both `https://lab.tootie.tv` and `https://lab.tootie.tv/mcp` as valid resources, plus trailing-slash normalization. Rejected: the client misconfiguration was the actual root cause; server relaxation would mask the error and weaken RFC 8707 compliance.
...
- Verify that Claude Code and codex successfully reconnect to `https://lab.tootie.tv/mcp` after the nginx reload — user was asked to test but session ended before confirmation.
...
- Add `lab.tootie.tv` to an explicit allowlist comment in `mcp.conf` or document the `LAB_MCP_ALLOWED_HOSTS` env var as the correct way to extend the allowed hosts list for the rmcp DNS rebinding protection (`crates/lab/src/cli/serve.rs:593–633`).
