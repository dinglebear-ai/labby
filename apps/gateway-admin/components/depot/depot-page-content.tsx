...
import { Check, Copy, Download, Grid2X2, Layers3, Link2, List, Loader2, PackagePlus, Search, X } from 'lucide-react'
...
export function DepotPageContent() {
  ...
  return <>
    ...
    <ArtifactInspection artifact={detail} loading={detailLoading} importing={importing} open={Boolean(selectedId&&selectedArtifactProvider)} closeHref={artifactHref()} copied={copied} onOpenChange={open=>{if(!open)router.push(artifactHref(),{scroll:false})}} onCopy={copyValue} onExport={exportArtifact} onImport={importArtifact}/>
  </>
}
...
function ArtifactInspection({artifact,loading,importing,open,closeHref,copied,onOpenChange,onCopy,onExport,onImport}:{artifact:FederatedArtifact|null;loading:boolean;importing:boolean;open:boolean;closeHref:string;copied?:string;onOpenChange:(open:boolean)=>void;onCopy:(label:string,value?:string)=>void;onExport:(artifact:FederatedArtifact)=>void;onImport:(artifact:FederatedArtifact)=>Promise<void>}){
