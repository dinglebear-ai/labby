...
| Count | Rule | Location |
|------:|------|----------|
| 228 | `missing_docs` (struct fields) | `crates/lab-apis/src/retired-upstream/types/*` |
| 52  | `clippy::unused_async` | `extract/transport.rs`, `retired-upstream/client/{movies,queue,indexers,download_clients,tags,history,...}.rs` stubs |
| 37  | `missing_docs` (enum variants) | `retired-upstream/types/command.rs` and siblings |
| 23  | `clippy::doc_markdown` | `core/auth.rs`, `core/plugin.rs`, `retired-upstream/**`, `openai/**` (product names without backticks) |
...
| 3   | `clippy::expect_used` | `core/http.rs:23`, `tests/http_client.rs:37`, `tests/retired-upstream_health.rs:48` |
| 3   | `clippy::missing_const_for_fn` | `extract/types.rs`, `api/error.rs`, `mcp/envelope.rs` |
| 3   | `clippy::cast_possible_truncation` | `retired-upstream.rs:93` + `retired-upstream.rs:100` (`as_millis() as u64`) |
...
| 2   | `clippy::struct_excessive_bools` | `retired-upstream/types/notification.rs:21`, `retired-upstream/types/system.rs:9` |
...
- `crates/lab-apis/src/retired-upstream.rs` — fix `cast_possible_truncation` in `ServiceClient::health` by using `.min(u64::MAX as u128)` or `u64::try_from(...).unwrap_or(u64::MAX)`.
- `crates/lab-apis/src/core/auth.rs`, `crates/lab-apis/src/core/plugin.rs`, `crates/lab-apis/src/retired-upstream/**`, `crates/lab-apis/src/openai/**` — `doc_markdown` backtick fixes.
...
- `crates/lab-apis/src/retired-upstream/types/notification.rs`, `crates/lab-apis/src/retired-upstream/types/system.rs` — `#[allow(clippy::struct_excessive_bools)]` on the two offending structs (Retired upstream's API literally has that many bool flags; refactoring the DTO is the wrong fix).
- Test files `crates/lab-apis/tests/http_client.rs`, `crates/lab-apis/tests/retired-upstream_health.rs` — replace `.expect("...")` with `.unwrap()` is NOT the answer (both are denied). Add `#![allow(clippy::expect_used)]` at the top of test files.
...
**Why:** 23 warnings. Product names (`Retired upstream`, `ByteStash`, `OpenAI`, `OpenAPI`, `retired-upstream`, `Memos`, `Linkding`, `Paperless-ngx`, etc.) appearing in `///` comments without backticks. Clippy wants them backticked so rustdoc formats them as code. Purely mechanical.

**Files (23 hit sites, condensed by file):**
- `crates/lab-apis/src/core/auth.rs:28` — `ByteStash`
- `crates/lab-apis/src/core/plugin.rs:51,53,…` — `Retired upstream`, `ByteStash`, `Linkding`, `Paperless-ngx`, etc.
- `crates/lab-apis/src/retired-upstream/client/download_clients.rs:11` — `Retired upstream`
- `crates/lab-apis/src/retired-upstream/client/quality.rs:32` — `OpenAPI`
...
| Before | After |
|--------|-------|
| `/// Usenet download client (Retired upstream, ...).` | ``/// Usenet download client (`Retired upstream`, ...).`` |
| `/// Client for the OpenAI API.` | ``/// Client for the `OpenAI` API.`` |
| `/// See the Retired upstream OpenAPI spec.` | ``/// See the Retired upstream `OpenAPI` spec.`` |
...
Run: `cargo test -p lab-apis --features "retired-upstream retired-upstream"` — all tests still pass.
...
### Task 6: `retired-upstream.rs` cast truncation (real correctness fix)
...
**Files:**
- Modify: `crates/lab-apis/src/retired-upstream.rs:93, 100`

- [ ] **Step 1: Read the current impl.**

Read `crates/lab-apis/src/retired-upstream.rs` lines 86-110.
...
Run: `cargo test -p lab-apis --features "retired-upstream retired-upstream"` — still green.

- [ ] **Step 4: Commit.**

```bash
git add crates/lab-apis/src/retired-upstream.rs
git commit -m "fix(retired-upstream): saturate latency_ms conversion instead of truncating"
```
...
**Why:** 2 hits in `tests/http_client.rs:37` and `tests/retired-upstream_health.rs:48`. Integration tests MUST fail loud on unexpected `None`/`Err` — `.expect(...)` is idiomatic. The workspace lint should be relaxed for `tests/` specifically.

**Files:**
- Modify: `crates/lab-apis/tests/http_client.rs:1`
- Modify: `crates/lab-apis/tests/retired-upstream_health.rs:1`
...
```bash
git add crates/lab-apis/tests/http_client.rs crates/lab-apis/tests/retired-upstream_health.rs
...
- [ ] **Step 4: `clippy::struct_excessive_bools` on `retired-upstream/types/notification.rs:21` and `retired-upstream/types/system.rs:9`.**

These are DTOs that mirror Retired upstream's API shape; the bools are fixed by upstream. Add a per-struct allow:
...
- `cast_possible_truncation` in `retired-upstream.rs` — Task 6 ✅
