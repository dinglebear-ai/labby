...
- `unknown_execution` — a Code Mode `resume_token` referenced no known durable run. MCP tool result error envelope. (A resume with a valid token but tampered integrity signature fails closed as `internal_error`; a resume whose resubmitted code hash mismatches the paused run fails as `resume_divergence`; a resume by a different actor or with narrowed/revoked live scope fails as `forbidden`.)
