  ...
  return <>
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-[10px] border-t border-[color-mix(in_srgb,var(--aurora-border-default)_60%,var(--aurora-page-bg))] bg-[var(--gw0-0_38)] px-4 pb-[11px] pt-[9px]">
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
        {onFork?<Button data-visible-label variant="outline" title={inLibrary ? 'Open upstream and fork options' : 'Fork into your Library'} aria-label={inLibrary ? 'Open upstream and fork options' : 'Fork into your Library'} onClick={()=>void onFork(artifact)} className="size-8 rounded-[9px] border-[color-mix(in_srgb,var(--aurora-accent-pink-deep)_42%,transparent)] bg-[color-mix(in_srgb,var(--aurora-accent-pink)_9%,transparent)] p-0 text-aurora-accent-pink"><GitFork aria-hidden className="size-3.5" strokeWidth={1.7}/></Button>:null}
        {onSend?<Button data-visible-label variant="outline" title={inLibrary ? 'Copy Library link' : 'Send to your Labby instance'} aria-label={inLibrary ? 'Copy Library link' : 'Send to your Labby instance'} onClick={()=>void onSend(artifact)} className="size-8 rounded-[9px] border-[color-mix(in_srgb,var(--aurora-accent-primary)_42%,transparent)] bg-[color-mix(in_srgb,var(--aurora-accent-primary)_9%,transparent)] p-0 text-aurora-accent-strong"><Send aria-hidden className="size-3.5" strokeWidth={1.7}/></Button>:null}
        {installFormats?.length&&onInstallFormat?<Popover><PopoverTrigger asChild><Button data-visible-label variant="outline" title={inLibrary ? 'Export artifact' : 'Install formats — compiled by APM'} aria-label={inLibrary ? 'Export artifact' : 'Install formats'} className="size-8 rounded-[9px] border-aurora-success/40 bg-aurora-success/10 p-0 text-aurora-success"><Download aria-hidden className="size-3.5" strokeWidth={1.7}/></Button></PopoverTrigger><PopoverContent align="end" side="top" sideOffset={5} className="box-content w-[236px] rounded-[12px] border-[color-mix(in_srgb,var(--aurora-border-strong)_75%,var(--aurora-page-bg))] bg-[linear-gradient(180deg,var(--aurora-panel-strong-top),var(--aurora-panel-strong))] p-[5px] shadow-[var(--aurora-shadow-strong),inset_0_1px_0_rgba(255,255,255,0.05)]"><span className="block px-2 pb-[3px] pt-1 text-[9px] font-bold uppercase tracking-[0.13em] text-[#97b6c9]">{inLibrary ? 'Export' : 'Compile Via APM'}</span>{installFormats.map(format=><PopoverClose asChild key={format}><Button data-visible-label variant="ghost" className="flex w-full items-center justify-start gap-2 rounded-lg px-2 py-[7px] text-[11.5px] font-[650]" onClick={()=>void onInstallFormat(artifact,format)}><PreviewTargetMark label={format}/><span className="truncate">{format}</span></Button></PopoverClose>)}</PopoverContent></Popover>:null}
