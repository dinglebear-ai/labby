...
| `pool/listing_bounds_tests.rs` | Focused regressions for bounded catalog listing passes: looping/endless-cursor mock upstreams, truncation visibility in status, and subject-scoped tier pinning (resources, prompts, prompt owner, gateway schema). |
...
| `pool/paginate.rs` | Bounded pagination for catalog listing RPCs (`list_*_bounded`): at most `MAX_LIST_PAGES` (16) pages per upstream per pass, early stop on a repeated `nextCursor`, WARN + `ListTruncation` report on truncation. Catalog-publishing callers surface the report — refresh passes atomically via `record_listing_success_for`, connect-time discovery via the new entry's `tool_last_error` — so `gateway.status` shows the partial state; subject-scoped per-subject listings rely on the WARN. Also owns `listing_catalog_timeout` / `with_listing_timeout` — the shared per-upstream wall-clock caps for listing fan-outs. Every upstream listing path (tools, prompts, resources, templates — connect-time discovery included) must use these instead of rmcp's unbounded `Peer::list_all_*`; `/clippy.toml` bans the `list_all_*` methods outright. |
...
- **SEP-2243 tool-header recovery is exact and single-shot.** rmcp derives
  ...
  independent schema caches. On the standard HeaderMismatch MCP error (`-32020`
  with the `header mismatch` marker), the tool-call primitive re-lists tools on
  that exact peer to refresh rmcp's transport-local schema cache and retries the
  same request once under the original deadline. Do not synthesize parameter
  headers from arbitrary arguments, broaden the matcher to ordinary MCP errors,
  or add an unbounded retry loop: annotated headers are a selective privacy
  boundary and HeaderMismatch is the pre-dispatch signal that makes one replay
  safe.
