...
export function TaskScheduleForm({ detail, agents, pending, error, onClose, onSave }: { detail?: TaskScheduleDetail; agents: AgentView[]; pending: boolean; error?: string; onClose: () => void; onSave: (draft: ScheduleDraft, spec: ScheduleSpec) => Promise<void> }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open && !pending) onClose() }}><DialogContent className="sm:max-w-[580px] border-aurora-border-strong bg-aurora-panel-strong text-aurora-text-primary" showCloseButton={!pending}><DialogHeader><DialogTitle>{detail ? 'Edit Task' : 'New Task'}</DialogTitle><DialogDescription className="text-aurora-text-muted">Schedule an active agent in this workspace. Its pinned harness, loadout, and environment determine execution.</DialogDescription></DialogHeader>
