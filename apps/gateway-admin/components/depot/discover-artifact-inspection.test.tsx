...
test('inspector renders revision file counts and retains the exact artifact import action', async () => {
  ...
  try {
    ...
    imported = undefined
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={artifact} inLibrary />)
    const presentButton = [...document.querySelectorAll('button')].find(node => node.textContent === 'In Library')
    assert.ok(presentButton)
    assert.equal(presentButton.disabled, true)
    await act(async () => { presentButton.click() })
    assert.equal(imported, undefined)
    let sent: FederatedArtifact | undefined
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={artifact} inLibrary onSend={value => { sent = value }} />)
    const sendButton = [...document.querySelectorAll('button')].find(node => node.textContent === 'Send to Labby')
    assert.ok(sendButton)
    assert.equal(sendButton.disabled, false)
    await act(async () => { sendButton.click() })
    assert.equal(sent, artifact)
    assert.equal(imported, undefined)
    let forked: FederatedArtifact | undefined
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={artifact} onFork={value => { forked = value }} />)
    const forkButton = [...document.querySelectorAll('button')].find(node => node.textContent === 'Fork')
