...
import { acceptGeneration, acceptGrantPage, acceptRecipientSearch, copyUri, mergeFiles, mergeGrants, selectedRecipientId, stashFileKind } from './view-state.ts'
...
test('file kind is an extension hint with case-insensitive categories and an unknown fallback', () => {
  assert.equal(stashFileKind('report.PDF'), 'Doc')
  assert.equal(stashFileKind('records.jsonl'), 'Data')
  assert.equal(stashFileKind('main.rs'), 'Code')
  assert.equal(stashFileKind('capture.webp'), 'Image')
  assert.equal(stashFileKind('bundle.tar.gz'), 'Archive')
  assert.equal(stashFileKind('opaque'), 'File')
})
