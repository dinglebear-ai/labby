...
Pairing creates a non-extractable Ed25519 private `CryptoKey`. The extension displays a short fingerprint derived from the pending request and that public key; the operator must supply the same fingerprint when approving the pairing in Labby. This binds approval to the browser that initiated the request instead of trusting the request name or WebSocket Origin alone. The key is preserved by IndexedDB structured cloning and is never exported to `chrome.storage.local`; that storage contains only non-secret settings and server pairing/association identifiers. Copying ordinary extension settings therefore does not produce an exportable signing credential, but this is not an OS hardware-keystore guarantee: anyone controlling the browser profile and browser process may still be able to use the key through WebCrypto.
