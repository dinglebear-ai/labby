...
External-service brand colors (for example, GitHub green, Tailscale blue, and Unraid orange) are kept in `lib/branding/service-brands.ts`, not inlined in components. They are the single sanctioned exception to the "no raw hex" rule because they encode third-party brand identity rather than Aurora UI tone.
