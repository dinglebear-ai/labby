...
| `pool/catalog_publication.rs` | Immutable publication of the routable tool, regular-resource, regular-resource-template, and regular-prompt projections; deliberately excludes UI resources/templates, skills, OAuth subjects, local and synthetic rows, and unrelated capability health. Also owns each upstream's retained resource snapshot (`ResourceSource`, regular and `ui://` rows validated and byte-budgeted together, stamped with `listed_at`), the cache-only listing `cached_upstream_resources_allowed`, and `cold_resource_snapshots`, which classifies snapshots as missing or stale for the warm-up. Each projection carries an independent generation so one catalog family cannot perturb another's publication identity. Owns the `CatalogWriteGuard` republish invariant: `catalog_write()` hands out the guard and its `Drop` calls `publish_if_changed()`, so no caller has to mark a projection dirty by hand. A `land:` merge silently dropped this module's `mod` declaration, which is what motivated the `xtask` module-reachability gate. |
