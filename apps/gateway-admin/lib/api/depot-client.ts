...
export async function listArtifacts(input: { provider?: string; sourceOrigin?: string; query?: string; limit?: number; cursor?: string; feed?: 'new' } = {}, signal?: AbortSignal): Promise<DiscoveryPage> {
  ...
  if (input.sourceOrigin !== undefined && !['mcp-registry', 'acp-registry', 'ard'].includes(input.sourceOrigin)) throw new Error('Invalid source origin')
  const page = await requestV2('/v1/depot/discover', { method: 'POST', signal, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ provider: provider === 'all' ? null : provider, sourceOrigin: input.sourceOrigin, query, limit: input.limit ?? 50, cursor: input.cursor, feed: input.feed }) }, discoverySchema, 'discovery response', 'retry-once')
  if (page.sourceOrigin !== input.sourceOrigin) throw new Error('Depot returned the wrong source origin scope')
  if (input.sourceOrigin && page.items.some(item => item.sourceOrigin !== input.sourceOrigin)) throw new Error('Depot returned an artifact from the wrong source origin')
  if (page.feed !== input.feed) throw new Error('Depot returned the wrong discovery feed')
  if (input.feed === 'new' && (!page.asOf || !page.rankingVersion || !page.feedCoverage)) throw new Error('Depot omitted New feed metadata')
  if (input.feed === 'new' && page.items.some(item => !item.firstSeenAt)) throw new Error('Depot omitted New feed observation')
  ...
  if (page.highlights) {
    if (provider !== 'all' || input.sourceOrigin || input.feed || query || input.cursor) throw new Error('Depot returned featured feeds outside the discovery landing scope')
    const participating = new Set(page.providerOutcomes.filter(outcome => outcome.state === 'participating' || outcome.state === 'exhausted').map(outcome => outcome.providerId))
    for (const feed of Object.values(page.highlights)) {
      if (feed.state !== 'ready') continue
      const identities = new Set<string>()
      for (const { artifact } of feed.items) {
        const identity = JSON.stringify([artifact.providerId, artifact.artifactId])
        if (!participating.has(artifact.providerId) || identities.has(identity)) throw new Error('Depot returned invalid featured artifact scope')
