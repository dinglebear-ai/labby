...
export function DiscoverSourceBadge({ artifact, plain = false }: { artifact: Pick<FederatedArtifact, 'providerId' | 'sourceOrigin'>; plain?: boolean }) {
  ...
  const title = source ? `${source.label} · via ${artifact.providerId}` : artifact.providerId
  if (plain) return <span className="inline-flex h-3 max-w-full items-center gap-[5px] text-[10px] font-[650] uppercase leading-3 tracking-[0.05em] text-aurora-text-muted" title={title}>
    <span aria-hidden="true" data-source-dot="1" className="size-1 shrink-0 rounded-full" style={{ background: color }} />
