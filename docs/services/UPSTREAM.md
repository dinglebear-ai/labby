...
| `lifecycle` | string | no | Handshake override. `auto` (default) tries `server/discover` and falls back to `initialize` when the peer proves it is legacy. `initialize` always opens with `initialize`, for legacy servers that exit on an unknown first request and so leave nothing to classify. `direct_stateless` keeps the modern 2026-07-28 stateless request model but satisfies `server/discover` locally for HTTP/Unix-socket servers that accept self-contained requests such as `tools/list` and `tools/call` while rejecting discovery. |
