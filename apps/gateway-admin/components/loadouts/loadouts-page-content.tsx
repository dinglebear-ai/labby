...
export function LoadoutsPageContent() {
  ...
  return <>
    ...
    <div className={cn(AURORA_PAGE_SHELL, 'flex-1')}><div className={AURORA_PAGE_FRAME}>
      ...
      <DashboardPanel title="Reusable capability projections" icon={<ShieldCheck className="size-4" />} action={<Button variant="outline" size="sm" asChild><Link href="/settings/surfaces#protected-mcp-routes">Mount on a route</Link></Button>}><p className={cn(AURORA_DENSE_META, 'text-aurora-text-muted')}>Loadouts define reusable projections; they do not create an endpoint by themselves. Mount one on an enabled protected MCP route to make it callable. Per-upstream exposure policies remain enforced underneath.</p></DashboardPanel>
