...
- [ ] **Verify ACP chat works end-to-end (smoke test the baseline)**
  ...
  Expected: a UUID. If it errors, fix the environment first (see prior session: `~/.lab/acp/codex-home/auth.json` must be in lockstep with `~/.codex/auth.json`).
...
- [ ] **Step 1: Write failing Rust test**
  ...
  fn sanitize_provider_error_strips_ip_path_and_secrets() {
      let raw = "failed to auth to 10.0.0.5:8000 with token=abc.def.ghi at /home/user/.lab/creds";
