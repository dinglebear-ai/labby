...
import { Check, Copy, Download, Grid2X2, Link2, List, Loader2, Lock, Search, X } from 'lucide-react'
...
import { ArtifactTypeMark } from './artifact-type'
import { artifactDescription, artifactKind, artifactLabel } from './library-model'
...
export function DepotPageContent() {
  ...
  return <>
    ...
    <div className={`${AURORA_PAGE_SHELL} flex-1`}><div className={AURORA_PAGE_FRAME}>
      <ConsoleHero eyebrow="Depot · Bazaar" title="Discovery" description="Find tools, agents, and reusable building blocks across the connected catalog." pulse={{color:state.status?.enabled?'var(--aurora-success)':'var(--aurora-warn)',label:state.status?.enabled?'connected':'disabled'}} actions={<Badge variant="outline" className="h-8 gap-1.5 border-aurora-warn/45 px-3 text-aurora-warn"><Lock className="size-3"/>Read-only</Badge>}>
        <div className="space-y-3"><p className="px-1 text-[11px] font-semibold text-aurora-text-muted">{state.loading?'Loading catalog…':`${resultCount.toLocaleString()} artifact${resultCount===1?'':'s'} ready to explore`}</p>
      ...
      <section aria-labelledby="artifact-results-title" className="space-y-3"><div className="flex items-end justify-between gap-3 px-0.5"><div><h2 id="artifact-results-title" className="text-base font-semibold text-aurora-text-primary">{activeQuery?`Results for “${activeQuery}”`:'Explore the catalog'}</h2>{!activeQuery?<p className="mt-1 text-xs text-aurora-text-muted">Recently indexed artifacts from every supported type.</p>:null}</div><span className="text-[11px] font-semibold text-aurora-text-muted">{state.loading?'Searching…':`${state.artifacts.length} of ${resultCount.toLocaleString()} shown`}</span></div>
        ...
        {state.cursor?<div ref={loadMoreRef} className="flex min-h-12 items-center justify-center" role="status" aria-live="polite">{state.loading?<><Loader2 className="mr-2 size-4 animate-spin"/>Loading more artifacts…</>:state.error?<Button variant="outline" onClick={()=>void load(activeQuery,state.cursor)}>Retry loading artifacts</Button>:<span className="text-xs text-aurora-text-muted">Scroll to load more</span>}</div>:state.artifacts.length?<p className="py-2 text-center text-xs text-aurora-text-muted" role="status">All {state.artifacts.length} artifacts loaded.</p>:null}
      </section>
    </div></div>
...
function ArtifactCard({artifact,compact,selected,href}:{artifact:DepotArtifact;compact:boolean;selected:boolean;href:string}){
  const label=artifactLabel(artifact),kind=artifactKind(artifact),namespace=artifact.namespace??artifact.descriptor?.namespace??'Unknown namespace'
  return <a href={href} aria-current={selected?'page':undefined} className={`group relative block overflow-hidden rounded-aurora-2 border bg-aurora-panel-medium shadow-sm transition-[transform,border-color,background-color] hover:-translate-y-0.5 hover:border-aurora-border-strong hover:bg-aurora-panel-strong aria-[current=page]:border-aurora-accent-primary ${compact?'px-4 py-3':'min-h-[220px] p-4'}`}><div className={compact?'grid items-center gap-4 md:grid-cols-[minmax(15rem,1.3fr)_minmax(10rem,1fr)_10rem]':'flex h-full flex-col'}><div className="min-w-0"><div className="flex items-center justify-between gap-2"><ArtifactTypeMark artifact={artifact} compact/>{artifact.publication?.state?<Badge variant="outline" className="max-w-28 truncate text-[8px] uppercase tracking-wider">{artifact.publication.state}</Badge>:null}</div><h3 className={`${compact?'mt-1.5':'mt-3'} truncate text-base font-semibold text-aurora-text-primary`}>{label}</h3>{!compact?<p className="mt-0.5 truncate text-[11px] text-aurora-text-muted">{namespace}</p>:null}</div>{compact?<div className="min-w-0"><p className="line-clamp-2 text-xs leading-5 text-aurora-text-muted">{artifactDescription(artifact)}</p><p className="mt-1 truncate text-[10px] text-aurora-text-muted">{namespace}</p></div>:<><p className="mt-3 line-clamp-2 min-h-10 text-xs leading-5 text-aurora-text-muted">{artifactDescription(artifact)}</p><div className="mt-3 flex gap-1.5"><Badge variant="outline">#{kind}</Badge>{artifact.publication?.visibility?<Badge variant="outline">{artifact.publication.visibility}</Badge>:null}</div></>}<div className={`${compact?'justify-end text-right':'mt-auto border-t border-aurora-border-subtle pt-3'} flex items-center justify-between gap-3 text-[10px] text-aurora-text-muted`}><span>{artifact.revisionCount===undefined?'Revisions —':`${artifact.revisionCount} revision${artifact.revisionCount===1?'':'s'}`}</span>{!compact?<span>{artifact.contentDigest||artifact.currentRevision?.contentDigest?'Digest verified':'Digest unavailable'}</span>:null}</div></div></a>
}
