...
Add more padding between card header text and the card's bottom border in the gateway admin UI. Also fix `lab extract` fleet scan — it was only finding credentials for 5 services from one host (`controller`) instead of scanning all SSH hosts and extracting all 9 supported services.
...
- **Opaque image IDs**: Docker stores only a hex hash (e.g. `5809619fa0e3`) when pulled by digest, discarding repository name. `overseerr` and `linkding` on `node-b` were invisible because of this.
...
| Services found | 5 (radarr, qbittorrent, tautulli, sonarr, sabnzbd on controller) | 9/9 across controller + node-b |
