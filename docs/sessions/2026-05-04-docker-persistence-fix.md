...
Identified and fixed a fundamental Docker persistence misconfiguration: the named volume covered `~/.local` but virtually all runtime state defaults to `~/.lab`. Replaced piecemeal per-path env var workarounds with a single bind-mount of the host's `~/.lab` directory. Also generated and wrote `LAB_ACP_HMAC_SECRET` to `~/.lab/.env` to prevent ACP permission signature invalidation on restart. Resolved a stale incremental build artifact error that blocked `just dev-debug` after the config changes.
...
3. Identified the initial problem: auth DB (`auth.db`) and JWT key (`auth-jwt.pem`) defaulted to `~/.lab/` but the named volume only covered `~/.local/`
...
7. Discovered that `acp.db`, `registry.db`, `stash/`, `node-enrollments.json`, `node-token` also defaulted to `~/.lab/` and were all ephemeral
8. Identified that `LAB_ACP_HMAC_SECRET` was unset, causing HMAC key rotation on every restart
9. Replaced the partial fix with a full bind-mount of `${HOME}/.lab:/home/lab/.lab`, removed the per-path env var workarounds
10. Generated `LAB_ACP_HMAC_SECRET` via `openssl rand -hex 32` and wrote it to `~/.lab/.env`
...
- `crates/lab-auth/src/config.rs:221` — `default_auth_dir()` returns `$HOME/.lab/`, not `$HOME/.local/share/lab/`
- `crates/lab/src/dispatch/acp/persistence.rs:1088-1096` — `resolve_db_path()` defaults to `$HOME/.lab/acp.db`; env override is `LAB_ACP_DB`
- `crates/lab/src/config.rs:1021-1026` — `registry_db_path()` is hard-coded to `$HOME/.lab/registry.db` with no env override
...
- `docker-compose.yml:29` — previous `${HOME}/.lab/acp:/home/lab/.lab/acp` mount only covered the `acp/` subdirectory, not `acp.db` one level up

## Technical Decisions

**Bind-mount `~/.lab` rather than redirecting per-path via env vars**
The alternative was adding `LAB_ACP_DB`, `LAB_REGISTRY_DB` env vars (registry.db has no override at all, requiring a code change). The bind-mount approach requires zero code changes, covers every current and future path that defaults to `~/.lab/`, and matches how local `labby serve` already uses the host's `~/.lab`. File-level mounts (`.env` `:ro`, `acp-providers.json` Docker-specific `:ro`) override the parent directory mount for those specific paths, preserving the existing access controls.

**Keep the named volume for logs only**
The log DB is ephemeral (TTL-pruned, 7-day retention) and redirected to `~/.local/share/lab/logs.db` via `LAB_LOCAL_LOGS_STORE_PATH`. Keeping it off the host's `~/.lab` avoids filling host storage with churn data.

**Remove the `acp` subdirectory bind mount**
`${HOME}/.lab/acp:/home/lab/.lab/acp` was redundant once the parent directory bind mount was added. Docker resolves this cleanly — the parent directory mount covers all paths including `acp/`.

## Files Modified

| File | Change |
|------|--------|
| `docker-compose.yml` | Added `${HOME}/.lab:/home/lab/.lab` bind mount; removed `${HOME}/.lab/acp:/home/lab/.lab/acp`; removed `LAB_AUTH_SQLITE_PATH` and `LAB_AUTH_KEY_PATH` env vars; updated comments |
| `.env.example` | Added `LAB_ACP_HMAC_SECRET=` with generation instructions |
| `~/.lab/.env` | Added `LAB_ACP_HMAC_SECRET=<generated>` (host file, not in-repo) |
...
|-----------|--------|-------|
| `auth.db` + `auth-jwt.pem` | Ephemeral (lost on restart) | Persisted via `~/.lab` bind mount |
...
**Shared host/container state:** Both local `labby serve` and the Docker container now write to the same `~/.lab/` directory. Running both simultaneously could cause SQLite write contention (WAL mode mitigates this but does not eliminate it). Mitigation: don't run local and Docker `labby serve` concurrently.

**acp-providers.json override:** The Docker-specific `./config/acp-providers.docker.json` is mounted as a file-level override at `/home/lab/.lab/acp-providers.json`. If Docker file-level mount semantics change or a compose version doesn't honor the override, the container could see the host's `~/.lab/acp-providers.json` instead. This is standard Docker behavior and not a real risk, but worth noting.

**Rollback:** Revert `docker-compose.yml` to the previous volume layout, re-add `LAB_AUTH_SQLITE_PATH`/`LAB_AUTH_KEY_PATH`, and restore the `acp` subdirectory mount. Remove `LAB_ACP_HMAC_SECRET` from `~/.lab/.env` if desired (reverting to ephemeral key).
