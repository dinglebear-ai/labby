...
Chat session metadata and persisted events survive a real backend/container restart when the caller uses the same authenticated principal. The tested session `c51ce807-a097-4aa3-8e70-dcc060858b17` was created through `/v1/acp/sessions`, prompted with `pwd`, persisted 54 events to `~/.labby/acp.db`, survived `docker restart lab-labby-master-1`, and replayed events 39-54 from `/v1/acp/sessions/{id}/events` after restart.
...
| Docker persistence | `docker-compose.yml:16` starts the volume list; `docker-compose.yml:21` maps `${HOME}/.labby` to `/home/labby/.lab`; `docker-compose.yml:31` mounts `.env` read-only | Default `acp.db` survives container restarts |
