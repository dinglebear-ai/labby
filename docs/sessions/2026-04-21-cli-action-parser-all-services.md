...
- **8 shims had `cli → mcp` dependency**: `apprise`, `arcane`, `memos`, `exampledownload`, `qdrant`, `tailscale`, `examplemetrics`, `openai` all called `crate::mcp::services::<svc>::dispatch` — forbidden per `src/CLAUDE.md`.
- **15 services have destructive actions** (confirmed via `grep "destructive: true"` on catalogs): `linkding`, `examplemedia`, `exampleindexer`, `examplerequests`, `paperless`, `exampleseries`, `apprise`, `arcane`, `memos`, `exampledownload`, `qdrant`, `tailscale`, `examplemetrics`, `exampleusenet`, `unraid` — but only `exampleusenet` and `unraid` previously had `-y/--yes` gates.
- **Pre-existing dead_code warnings**: `mcp/services/tailscale.rs` and `mcp/services/examplemetrics.rs` `dispatch` functions are never called — these services are not registered in `mcp/registry.rs`. The CLI-to-MCP dependency was masking this gap; my fix surfaced it.
...
2. **All destructive shims get `--dry-run`** — `cli/CLAUDE.md` requires `--dry-run` for destructive services; simpler `println!` pattern (matching `mcpregistry.rs`) rather than the `OutputFormat` match in old `exampleusenet`.

3. **`exampleusenet` dry_run simplified** — original had an `OutputFormat::Json` branch emitting structured JSON for dry-run; replaced with plain `println!` to match reference pattern and remove the special-case branch.
...
| `crates/lab/src/cli/examplemedia.rs` | Field type fix + `action_parser` + `run_confirmable` + `-y`/`--dry-run` |
| `crates/lab/src/cli/exampleindexer.rs` | Field type fix + `action_parser` + `run_confirmable` + `-y`/`--dry-run` |
| `crates/lab/src/cli/examplerequests.rs` | Field type fix + `action_parser` + `run_confirmable` + `-y`/`--dry-run` |
| `crates/lab/src/cli/paperless.rs` | Field type fix + `action_parser` + `run_confirmable` + `-y`/`--dry-run` |
| `crates/lab/src/cli/exampleseries.rs` | Field type fix + `action_parser` + `run_confirmable` + `-y`/`--dry-run` |
...
| `crates/lab/src/cli/exampledownload.rs` | Field type fix + `action_parser` + fix mcp→dispatch + `run_confirmable` + `-y`/`--dry-run` |
...
| `crates/lab/src/cli/examplemetrics.rs` | Field type fix + `action_parser` + fix mcp→dispatch + `run_confirmable` + `-y`/`--dry-run` |
| `crates/lab/src/cli/exampleusenet.rs` | Field type fix + `action_parser` + simplify `dry_run` branch |
...
# Result: 0 errors, 2 warnings (pre-existing dead_code in mcp/services/tailscale.rs + examplemetrics.rs)

# Runtime verification — all 20 services
for svc in apprise arcane bytestash gotify linkding memos openai examplerequests paperless examplemedia exampleindexer \
           exampledownload qdrant exampleusenet exampleseries tailscale examplemetrics tei unifi unraid; do
...
| `apprise`, `arcane`, `memos`, `exampledownload`, `qdrant`, `tailscale`, `examplemetrics` | Called `crate::mcp::services::<svc>::dispatch` (forbidden) | Calls `crate::dispatch::<svc>::dispatch` |
| `linkding`, `examplemedia`, `exampleindexer`, `examplerequests`, `paperless`, `exampleseries` | No `-y` / `--dry-run`; destructive actions ran without confirmation | `-y/--yes`, `--no-confirm`, `--dry-run` enforced |
| `exampleusenet` | `dry_run` had `OutputFormat` match with JSON branch | Simple `println!` pattern matching reference |
...
| `lab exampleseries --help \| grep "possible values"` | Full action list | 35 actions listed | ✅ |
...
- **Rollback path**: `git checkout crates/lab/src/cli/{apprise,arcane,linkding,memos,openai,examplerequests,paperless,examplemedia,exampleindexer,exampledownload,qdrant,exampleusenet,exampleseries,tailscale,examplemetrics,tei,unraid}.rs`
...
- **Suppressing dead_code warnings for mcp/services/tailscale + examplemetrics**: Rejected — the warnings correctly surface that these services are missing MCP registry registration.

---

## Open Questions

- `mcp/services/tailscale.rs` and `mcp/services/examplemetrics.rs` dispatch functions are dead code — these services are not registered in `mcp/registry.rs`. Should they be added to the registry?
- `exampleusenet` params default changed from `serde_json::Value::Null` to `serde_json::Value::Object(...)` — verify this doesn't affect any callers expecting null.

---

## Next Steps

- Register `tailscale` and `examplemetrics` in `mcp/registry.rs` (pre-existing gap now surfaced).
- Consider upgrading `linkding`, `examplemedia`, `exampleindexer`, and other dispatch-backed shims to typed `Subcommand` enums (Tier 1) if richer UX is needed.
