...
type EventView = {
  kind: 'reasoning' | 'tool' | 'plan' | 'diff' | 'usage' | 'warning' | 'error' | 'status'
  label: string
  detail?: string
}
...
function summarize(event: PhoenixEvent): EventView | undefined {
  ...
  const text = firstText(eventItem, ['text', 'summary', 'title']) ?? firstText(params, ['message', 'text', 'summary'])
  ...
  if (method.includes('token') || method.includes('usage')) {
    ...
    const input = typeof usage.inputTokens === 'number' ? usage.inputTokens : usage.input_tokens
    const output = typeof usage.outputTokens === 'number' ? usage.outputTokens : usage.output_tokens
    const detail = [typeof input === 'number' ? `${input.toLocaleString()} in` : '', typeof output === 'number' ? `${output.toLocaleString()} out` : ''].filter(Boolean).join(' · ')
...
export function PhoenixRuntimeSummary({ mcpConfigured, protocol }: { mcpConfigured?: boolean; protocol?: string }) {
  ...
  return <div aria-label="Phoenix runtime capabilities" className="flex min-w-0 items-center gap-1.5 overflow-hidden text-[9.5px] font-semibold text-aurora-text-muted">
    <span title={protocol ? `Codex App Server protocol ${protocol}` : undefined} className="h-5 shrink-0 rounded-md border border-aurora-border-default/60 bg-[var(--gw0-0_45)] px-2 leading-5">{protocol ? `v${protocol.split('-')[0]}` : 'App Server'}</span>
