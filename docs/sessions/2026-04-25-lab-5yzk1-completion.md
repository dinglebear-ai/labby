...
target/debug/lab retired-upstream series.list --dry-run
just check
just build
```
...
```bash
rg -n 'crate::dispatch::(apprise|arcane|linkding|memos|openai|retired-upstream|paperless|retired-upstream|retired-upstream|retired-upstream|qdrant|retired-upstream|retired-upstream|tailscale|retired-upstream|tei|unraid)::dispatch' crates/lab/src/cli/{apprise,arcane,linkding,memos,openai,retired-upstream,paperless,retired-upstream,retired-upstream,retired-upstream,qdrant,retired-upstream,retired-upstream,tailscale,retired-upstream,tei,unraid}.rs
```
...
retired-upstream: possible values ok
...
Command: `target/debug/lab retired-upstream series.list --dry-run`

Result: exit 0.

```text
[dry-run] would dispatch retired-upstream action `series.list` with params: {}
```
