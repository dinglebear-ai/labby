...
/** A self-contained monochrome mark for the embedded, offline-capable UI. */
export function LocalBrandMark({ name }: { name: string }) {
  const iconClass = 'size-full fill-current'
  let mark: React.ReactNode
  if (name === 'Ubuntu 24.04') mark = <UbuntuMark className={iconClass} />
  else if (name === 'Debian 12') mark = <DebianMark className={iconClass} />
  ...
  return (
    <span aria-hidden="true" className="grid size-8 shrink-0 place-items-center overflow-hidden rounded-aurora-1 bg-black p-1.5 text-[10px] font-black text-white">
      {mark}
    </span>
  )
}
