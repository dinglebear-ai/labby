...
- [x] **Wave 54 / Milestone 0BF protected Complete-only Tool handler mount:** branch on the middleware Project observation before destructive/scope dispatch; retain Legacy behavior, terminate Unavailable, classify Bound ownership from the same transport publication plus the central reserved predicate, and route only Bound regular names through Wave 53 with no fallback. The mounted branch inherits the lower seam's exact-name, permission, expiry, ABA, metadata, cancellation, and zero-RPC exclusions while remaining outside OAuth, relay, widget, elicitation, and task lifecycles. Tracked under `lab-3pyb6`.
