...
fn tombstone_source_matches_upstream(
    tombstone: &UpstreamImportTombstone,
    source: &labby_runtime::gateway_config::ImportSource,
) -> bool {
...
pub fn validate_code_mode(
    ...
    code_mode.validate().map_err(|e| match e {
        labby_runtime::gateway_config::ConfigError::InvalidCodeModeTimeout { .. } => {
            ToolError::InvalidParam {
                message: e.to_string(),
                param: "code_mode.timeout_ms".to_string(),
            }
        }
        labby_runtime::gateway_config::ConfigError::InvalidCodeModeMaxResponseBytes { .. } => {
            ToolError::InvalidParam {
                message: e.to_string(),
                param: "code_mode.max_response_bytes".to_string(),
            }
        }
        labby_runtime::gateway_config::ConfigError::InvalidCodeModeMaxResponseTokens { .. } => {
            ToolError::InvalidParam {
                message: e.to_string(),
