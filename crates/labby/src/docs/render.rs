...
pub fn cli_help() -> String {
    let mut out = String::from("# Labby CLI Help\n\n");
    out.push_str("Generated by `labby docs generate` from the executable Clap command tree. Do not edit by hand.\n\n");
    let mut command = crate::cli::Cli::command().color(clap::ColorChoice::Never);
    command.build();
    write_cli_help(&mut out, "labby", &command.render_long_help().to_string());
    for entry in crate::cli::help::inventory() {
        write_cli_help(&mut out, &entry.command, &entry.help);
    }
    while out.ends_with("\n\n") {
        out.pop();
    }
...
/// Retirement advice is documentation only; these spellings are not accepted aliases.
pub fn cli_migration() -> String {
    let mut out = header("CLI command migration", "labby docs generate");
    out.push_str("This is a breaking command-grammar migration. Old command paths are rejected with replacement advice. Flags keep standard hyphenated syntax; resource names can contain hyphens.\n\n");
    out.push_str("| Retired command prefix | Canonical command prefix |\n| --- | --- |\n");
    for (old, new) in crate::cli::migration::MOVED {
        writeln!(out, "| `labby {old}` | `labby {new}` |").ok();
    }
    ...
    out.push_str("`host update` retains the existing host-and-Incus workflow unless Incus synchronization is explicitly disabled. `host update auto` selects the native schedule operation. `server auth login` opens the browser by default; use `--no-browser` for URL-only authorization. Provider-wide revocation remains separate under `auth provider`.\n\n");
    out.push_str("`logs` now returns a bounded local rolling-log query. `logs journal` explicitly selects the deployment journal, and `--follow` enables streaming. Journal output is raw; use `logs --json` for structured results. Never retry an uncertain Code Mode result by switching targets.\n");
    out
}
...
fn write_cli_help(out: &mut String, path: &str, help: &str) {
    writeln!(out, "## `{path}`\n\n```text").ok();
