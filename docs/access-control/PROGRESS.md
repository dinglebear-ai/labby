...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. Implementation includes an unmounted manager composition of coherent runtime Loadout, runtime-pool, and upstream tool-catalog observations plus a separate generated built-in service/action registry publication. These are not yet one complete gateway catalog or dispatch grant, and access-control enforcement is not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
...
- [ ] exact Project + policy epoch/complete catalog generation cache key. Waves 16 and 17 separately publish runtime Loadout/upstream-tool facts and built-in service/action facts, but they are not yet composed with virtual-server policy, subject OAuth catalogs, other MCP capability families, exact targets, or the final dispatch boundary.
