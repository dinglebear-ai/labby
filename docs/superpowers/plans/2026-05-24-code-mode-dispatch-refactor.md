...
async fn search_expands_builtin_matches_to_action_candidates() {
    ...
    assert_eq!(results.first().map(|r| r.id.as_str()), Some("lab::examplemovies.movie.search"));
...
async fn schema_returns_lab_action_bindings() {
    ...
    assert!(schema.bindings.typescript.contains("callTool(\"lab::examplemovies.movie.search\""));
}
```
...
async fn search_keeps_builtin_candidates_when_upstream_search_is_unavailable() {
    ...
    assert!(results.iter().any(|r| r.id == "lab::examplemovies.movie.search"));
}
```
...
assert!(Cli::try_parse_from(["lab", "gateway", "code", "schema", "lab::examplemovies.movie.search"]).is_ok());
