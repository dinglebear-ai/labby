...
retiring `.retired-upstream`/`.lab` in favor of `.retired-upstream`/`.labby`, (3) diagnosing and fixing a
...
5. **Dispatched 9 parallel research agents** (OpenWiki+beads/Dolt, squirts SSH review,
   lavra, retired-upstream-rmcp node packaging + xtask/mcporter, template-rmcp xtask patterns +
...
9. **Full chezmoi dotfiles reconciliation** — inspected drift across ~13 files
   ...
   `run_onchange` scripts manually, left one cosmetic `.retired-upstream` permission-mode
   mismatch unresolved (flagged, not fixed).
10. **User retired `.retired-upstream`/`.lab`, added `.retired-upstream`/`.labby`** — `chezmoi forget`
...
- `~/.retired-upstream` had genuinely diverged live-vs-source permission bits (`750`/`640` live vs.
  ...
  `.retired-upstream` was subsequently retired by the user in favor of `.retired-upstream`).
...
- Did not force through the `.retired-upstream` permission-mode diff during chezmoi reconciliation
...
- **Agent tool (parallel research)**: 9 concurrent research agents dispatched for the
  fleet-documentation task (OpenWiki/beads/Dolt, squirts SSH, lavra, retired-upstream-rmcp packaging
...
| `.retired-upstream` / `.lab` (dotfiles) | chezmoi-managed | Unmanaged (live directories untouched, safe to delete later) |
| `.retired-upstream` / `.labby` (dotfiles) | Unmanaged | Config/`.env` files tracked, encrypted |
...
- **Restructure the workspace to give every crate an explicit `version = "x.y.z"`**
  ...
  over the chosen fix.
- **Force-applying the `.retired-upstream` chezmoi permission-mode diff** during dotfiles
  ...
  (moot now that `.retired-upstream` has since been retired).
