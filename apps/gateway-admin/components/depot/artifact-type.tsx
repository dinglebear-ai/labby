import { AppWindow, BookOpen, PackageSearch } from 'lucide-react'
...
export const ARTIFACT_TYPES: ArtifactType[] = ['mcp', 'acp', 'agent', 'skill', 'command', 'plugin', 'marketplace', 'prompt', 'resource', 'app']
...
const LABELS: Record<string, [string, string]> = {
  ...
  resource: ['Resources', 'Resource'], app: ['Apps', 'App'],
...
export function artifactTypeDefinition(kind: string) {
  ...
  if (key === 'resource') {
    const color = '#72dcc5'
    return { ...presentation, label, singularLabel, color, tone: color, icon: BookOpen,
  ...
  if (key === 'app') {
    const color = '#7cc7ff'
    return { ...presentation, label, singularLabel, color, tone: color, icon: AppWindow,
      iconStyle: { color, backgroundColor: `color-mix(in srgb, ${color} 10%, transparent)`, borderColor: `color-mix(in srgb, ${color} 30%, transparent)` } }
  }
