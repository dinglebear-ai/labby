...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. GatewayManager now combines the generation-bound Loadout-filtered upstream-tool and built-in MCP service/action projections over one bounded common publication interval. This unified MCP snapshot is not yet composed with the stable Project runtime context or into a dispatch grant, and access-control enforcement is not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
...
- [ ] exact Project + policy epoch/complete catalog generation cache key. Wave 19 unifies the generation-bound Loadout-filtered upstream-tool and built-in service/action facts, but that snapshot is not yet composed with the stable Project context, subject OAuth catalogs, other MCP capability families, exact targets, or the final dispatch boundary.
