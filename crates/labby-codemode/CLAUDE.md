...
| Process-group guard | Runner transport spawned with `process_group(0)` (Unix) / Job Object (Windows); `kill_on_drop(true)`; `killpg` reaches descendants. Windows Job Object assignment fails runner startup rather than silently degrading, while the sanctioned `labby-winjob` boundary documents the accepted spawn-to-assignment race. A Microsandbox runner also owns a cleanup guard that attempts force-removal of its named microVM on eviction/drop. |
