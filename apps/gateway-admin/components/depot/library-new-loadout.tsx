...
export function LibraryNewLoadout() {
  ...
  return <>
    <Button data-visible-label="1" variant="outline" disabled={!canManage} title={!canManage ? 'Loadout management requires scope management access' : undefined} onClick={() => setOpen(true)} className="h-9 gap-[7px] rounded-[10px] px-4 text-[13px] font-[650] text-aurora-accent-strong" style={{ borderColor: 'color-mix(in srgb, var(--aurora-accent-primary) 55%, var(--aurora-border-strong))', background: 'color-mix(in srgb, var(--aurora-accent-primary) 9%, var(--aurora-panel-strong))' }}><Layers size={14}/>New Loadout</Button>
