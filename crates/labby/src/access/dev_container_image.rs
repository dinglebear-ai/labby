...
impl AccessStore {
    ...
    pub(crate) async fn dev_container_build_enqueue(
        ...
        self.with_connection(move|connection|{
            ...
            if let Some(existing)=existing { if existing.template_id==input.template_id && existing.source_revision.eq(&draft.revision) && existing.source_digest==input.source_digest && existing.request_kind==input.request_kind {tx.commit().map_err(map_sqlite_error)?;return Ok(existing);} return Err(AccessStoreError::NotAuthorized); }
