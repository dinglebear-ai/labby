...
export function TopToolsChart({ tools, onSelect }: { tools: ToolUsageEntry[]; onSelect?: (tool: string) => void }) {
  ...
  return <ol aria-label="Top tools by supplied order" className="flex min-w-0 flex-col gap-2.5">
    {tools.map((tool, index) => {
      const label = readableTarget(tool.label ?? tool.name)
      const content = <>
      ...
      return <li key={tool.id ?? tool.name} className="min-w-0">{onSelect
        ? <button type="button" onClick={() => onSelect(tool.name)} aria-label={`Inspect ${label}: ${tool.calls} calls, ${tool.failed} failed`} className="-mx-2 block w-[calc(100%+16px)] min-w-0 rounded-lg px-2 py-1 text-left transition-colors duration-150 hover:bg-aurora-hover-bg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-aurora-accent-primary">{content}</button>
