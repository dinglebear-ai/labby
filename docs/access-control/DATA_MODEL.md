...
The implemented Project Loadout compatibility mutation accepts a caller-admitted canonical Loadout name and assigns it only to an active Project for which the exact active `VerifiedIdentity` has an active direct membership carrying `project.manage`. The AccessStore does not inspect gateway configuration or infer a default Loadout. The unmounted application composition adapter authorizes the Project before admitting the name against desired gateway configuration through `GatewayManager::loadout_get`, then the mutation reauthorizes before writing. An absent mapping is inserted in one immediate transaction with the global revision, owning Organization policy epoch, Project policy epoch, and a redacted audit event. Repeating the exact mapping is a zero-write `AlreadyApplied`; attempting to replace it with a different name fails with a typed conflict. Inaccessible and cross-Organization Projects remain non-enumerating failures. Admission is point-in-time: every later consumer must resolve the symbolic name from the coherently published runtime configuration and fail closed if it is absent or the publication changed; repair/replacement remains future work. Gateway now supplies that Loadout snapshot with an opaque process-local publication generation under its publication barrier. The value is a runtime-configuration generation, not yet the complete pool/catalog generation required for final dispatch.

The implemented unmounted use-time composer performs bounded A-G-A-G reads: Project permission snapshot, published runtime Loadout snapshot, Project permission snapshot, then published runtime Loadout snapshot. It returns a context only when both complete Access snapshots and both complete Gateway snapshots match. Sustained Access or Gateway churn fails closed after three attempts. This optimistic cross-store check cannot prevent a mutation immediately after the final read, so its non-cloneable result is not reusable execution authority; final dispatch still requires exact-action reauthorization and the complete gateway catalog/runtime boundary.
