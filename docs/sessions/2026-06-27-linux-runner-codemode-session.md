...
# Code Mode upstream context and controller Linux runner setup
...
This session produced Code Mode upstream namespace visibility work, documented the remaining enrichment-hints plan, and then set up a Linux self-hosted GitHub Actions runner on controller. The runner now executes the Linux `Test` lane successfully, keeps `/tmp` off Unraid RAM, and has a hard 60G ZFS quota plus startup pruning so runner state cannot grow without bound.
...
5. Set up a Linux self-hosted runner on controller in Docker Compose, moved persistent runner state to Unraid cache paths, and verified the Linux CI `Test` job on `linux-ci-runner`.
...
- The Linux self-hosted runner is online in GitHub as `linux-ci-runner` with labels `self-hosted,X64,linux-ci,linux`; `workstation-lab` remains offline.
...
- CI run `28278942503` proved the Linux `Test` job passed on controller: job `83790978353`, `cargo nextest run --workspace --all-features --locked --profile ci`, started `2026-06-27T04:45:44Z`, completed `2026-06-27T04:50:20Z`.
...
- Put controller's runner Compose and startup script under `/mnt/cache/compose/actions-runner/lab/` because `/opt` is not persistent on Unraid.
...
| modified | `.github/actionlint.yaml` |  | Accept `linux-ci` self-hosted label | commit `b45e7e10` |
| modified | `.github/workflows/ci.yml` |  | Route trusted Linux test lane to controller and keep fork fallback on GitHub-hosted runners | commit `b45e7e10` |
| created | `docs/runtime/ACTIONS_RUNNER.md` |  | Document controller runner setup, deps, quota, and pruning | commits `b45e7e10`, `9205fc91`, `90e2c11b` |
...
The latest Claude transcript path existed, but its content was an older Aurora theme session from 2026-05-31. This session log therefore uses the current Codex thread context plus live git/GitHub/controller evidence for the current session facts.
...
- **Shell commands.** Used for git status/logs, GitHub CLI inspection, controller SSH checks, Docker runner logs, ZFS quota verification, and Beads reads.
- **GitHub CLI.** Used to inspect Actions runs, runner registrations, open PRs, and CI job status.
- **Remote SSH to controller.** Used to create/migrate the persistent runner appdata dataset, manage Docker Compose, verify mounts, and inspect runner state.
- **Docker/Compose.** Used on controller for the `lab-linux-runner` container.
...
| `gh api repos/jmagar/labby/actions/runners --paginate --jq ...` | Confirmed `linux-ci-runner` online and `workstation-lab` offline. |
| `gh run view 28278942503 --json jobs,status,conclusion,url,createdAt,updatedAt` | Confirmed Linux `Test` passed on controller and identified remaining CI blockers. |
| `ssh controller 'zfs list ...; du -sh ...; docker exec lab-linux-runner df -h ...'` | Confirmed the runner dataset quota, sizes, and mounted paths. |
...
- The first controller runner attempt lacked native build dependencies such as `cc`, so `start.sh` gained dependency bootstrap for `build-essential`, `pkg-config`, `cmake`, `clang`, `libclang-dev`, and `nasm`.
...
| Linux CI test lane | Linux tests used GitHub-hosted runners or had no dedicated lab Linux runner. | Trusted Linux tests can run on controller via `linux-ci`; fork PRs still fall back to GitHub-hosted `ubuntu-latest`. |
...
|---|---|---|---|
| `gh api repos/jmagar/labby/actions/runners --paginate --jq ...` | `linux-ci-runner` online with `linux-ci` | `49 linux-ci-runner online false self-hosted,X64,linux-ci,linux` | pass |
| `ssh controller 'zfs list ... cache/appdata/actions-runner/lab'` | Dedicated dataset has a hard quota | `quota 60G`, `used 2.00G`, `available 58.0G` | pass |
| `ssh controller 'docker exec lab-linux-runner df -h /tmp /home/runner /home/runner/_work'` | Runner temp/home/work mount to bounded dataset | all three paths mounted on `cache/appdata/actions-runner/lab` | pass |
...
- Bring `workstation-lab` back online or temporarily route Windows CI to a runner that is available.
...
- Monitor controller's runner dataset after several CI runs with `zfs list -o name,quota,used,available cache/appdata/actions-runner/lab`.
