...
This is the operational entry point for Labby's isolated verification workspace.
The durable architecture and qualification boundaries live in
[Verification and Compliance](../../docs/dev/VERIFICATION.md). The core parses
...
The generated [invariant schema](schemas/invariants.schema.json) and
[scenario schema](schemas/scenario.schema.json) are canonical workspace
artifacts. Tests compare them byte-for-byte with freshly generated output and
validate positive/negative wire examples. Namespace equality, registry resolution, nonblank strings, uniqueness,
...
`labby-verify t1 tools/verification/formal` resolves the five registered Stateright handles, then
...
cargo run --manifest-path tools/verification/Cargo.toml -p verify-runner --example replay-fixture --locked -- replay path/to/fixture.json
```
...
```sh
cargo run --manifest-path tools/verification/Cargo.toml -p labby-verify -- incident lifecycle.json LABBY-REQ-005
cargo run --manifest-path tools/verification/Cargo.toml -p labby-verify -- incident-explore lifecycle.json LABBY-REQ-005
```
