...
| `pool/paginate.rs` | Bounded pagination for catalog listing RPCs (`list_*_bounded`): at most `MAX_LIST_PAGES` (16) pages per upstream per pass, early stop on a repeated `nextCursor`, WARN + truncation counter on truncation. Catalog listing paths must use these instead of rmcp's unbounded `Peer::list_all_*`. |
