...
Built two complete interactive HTML mockups for the Setup wizard and Settings page, wired them to the live Rust backend via a new `/dev/api/retired-dev-route` endpoint, replaced the Labby logo with a network-node SVG, conducted a code review with three findings fixed, and produced two detailed React implementation plans totaling 23 tasks covering the complete Tier 2 migration.
...
11. Added `/dev/api/retired-dev-route` — unauthenticated Rust endpoint returning `local_host`, `controller`, `master_url` from config.toml plus masked env vars from process environment
12. Pre-populated all wizard fields from live env (RETIRED_UPSTREAM_URL, RETIRED_UPSTREAM_TOKEN, LAB_MCP_HTTP_TOKEN, etc.)
...
- `dotenvy::from_path` loads env vars into the process environment at startup; `std::env::vars()` in the `dev_retired-dev-route` handler correctly reads them without pre-sourcing the `.env` file
...
- **`/dev/api/retired-dev-route` unauthenticated**: The setup wizard runs before auth is configured; the endpoint only exposes non-secret config values and masked secrets, making unauthenticated access acceptable.
...
| File | Change |
|------|--------|
| `crates/lab/src/api/router.rs` | Added `dev_mockup`, `dev_mockup_named`, `dev_retired-dev-route` handlers; fixed XSS, secret masking, misleading comment |
...
| `docs/design/component-development.md` | Added two-tier serving model, retired-dev-route endpoint pattern |
...
# Verify retired-dev-route returns env values
wget -qO- http://localhost:8765/dev/api/retired-dev-route | python3 -c "..."
# Result: env keys: 52, RETIRED_UPSTREAM_URL: http://100.64.0.11:7878
...
| `env keys: 0` in retired-dev-route | Binary compiled from stripped version of router.rs; also initial attempt read from file instead of process env | Rebuilt binary; switched to `std::env::vars()` |
...
| `/dev/api/retired-dev-route` | Not present | Returns hostname, controller, master_url, masked env vars |
...
| `wget -qO- http://localhost:8765/dev/api/retired-dev-route` | JSON with controller="node-a" | `{"local_host":"node-a","controller":"node-a","master_url":"http://node-a:8765","env":{...52 keys...}}` | ✅ |
...
- **Unauthenticated retired-dev-route**: `/dev/api/retired-dev-route` is intentionally unauthenticated but returns service topology (URLs, hostnames). Acceptable for localhost-only setups; risk increases if `LAB_MCP_HTTP_HOST=0.0.0.0` without auth.
...
5. **Tests for `dev_retired-dev-route`** — code review flagged missing tests for auth bypass, masking, and prefix filtering (deferred)
