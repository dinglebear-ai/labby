...
> description of current code. The access store is now at schema v8 (see
> [DATA_MODEL.md](./DATA_MODEL.md#schema-v8-current) and
...
Listing returns only active same-Organization direct memberships and includes an optional persisted Loadout name so callers can distinguish discoverable Projects from selectable Projects; a valid Principal may receive an empty list. Identity resolution and explicit selection fail closed through typed redacted errors for missing or unusable required records. The implementation issues no per-result queries, and the revision plus all returned facts come from the same SQLite read transaction; fixed query-count instrumentation remains an enforcement-readiness gate. This wave was read-only and did not itself install the snapshot into runtime state or authorize dispatch. Later completed waves added the protected regular MCP adapters described in the current progress ledger; other surfaces and capability families remain deferred.
...
Wave 43 added an unmounted, crate-private resolver/executor for regular non-OAuth Prompts; Wave 44 later mounted it through the protected transport wrapper. It derives fresh `AssetUse` authorization and coherent Project route/Loadout/catalog evidence before dispatch, resolves exactly one canonical `upstream/native` Prompt route, rewrites only the outbound upstream name to the native name, and rechecks the same authorization/publication interval before exposing the result. The manager brackets final Prompt outcome attribution with its pool-publication lease, so pool replacement or ABA cannot apply stale health or return stale output.

Authorization is checked when the upstream request starts and again before its result is exposed; it is not a revocation lease and cannot unsend a request already in flight. Pool and Prompt generations remain observations rather than capabilities. At this wave the inner seam did not bind a protected transport token instance or expiry; Wave 44 supplied middleware-derived `VerifiedIdentity` and transport validation at the mounted handler. Built-ins, OAuth/subject-scoped Prompts, and capability relay remain deferred for Bound routes.
...
Add one unmounted terminal adapter above Wave 52 for the regular pooled connection's truthful no-input-capabilities contract. Complete responses pass through unchanged. Any Task, InputRequired, or future non-Complete success variant that reaches the adapter is immediately discarded and becomes one static redacted `UnsupportedTerminalResponse`; the payload is not retained in the error, retried, relayed, registered as a task, or exposed. With the current rmcp pooled client, capability/protocol enforcement may instead reject Task or InputRequired on the upstream wire before a success variant reaches the adapter. Those already-sanitized MCP errors pass through unchanged: Wave 53 does not infer response provenance from spoofable error codes or messages. Wave 52's post-dispatch transport expiry and identity validation run before terminal classification, so `Unavailable` wins whenever the binding expires in flight. Other redacted MCP and transport errors likewise pass through unchanged while the binding remains valid.
...
Mount the Wave 53 Complete-only adapter at the beginning of `tools/call` for middleware-observed Project transports. `Legacy` requests retain the existing handler order and raw proxy behavior. `Unavailable` observations terminate as a static redacted Complete error. Every valid `Bound` call enters the exact regular AssetUse adapter and terminates without fallback. Centrally reserved names and services published by the bound loadout remain discoverable but fail closed at execution until built-ins and synthetics gain their own exact AssetUse reauthorization seam; global registry membership never grants Project execution authority.
