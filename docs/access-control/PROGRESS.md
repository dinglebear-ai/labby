...
- [ ] use-time Gateway Loadout resolution and repair/replacement workflow. Gateway and AccessStore are separate stores, so admission is not a durable existence invariant: concurrent or later removal can invalidate the symbolic name. Every consumer must re-resolve it and fail closed; replacement semantics remain unimplemented.
