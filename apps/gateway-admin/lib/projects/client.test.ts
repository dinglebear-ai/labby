...
test.afterEach(()=>{globalThis.fetch=originalFetch;__setBrowserSessionStateForTests({status:'unauthenticated'})})
test('project lifecycle uses authoritative endpoint and csrf for mutation',async()=>{authenticate();const requests:Request[]=[];globalThis.fetch=async(input,init)=>{const request=new Request(new URL(String(input),'http://labby.test'),init);requests.push(request);return Response.json(requests.length===1?[]:{project_id:'p'})};await listProjects();await createProject('team','p','Project');assert.equal(new URL(requests[0]!.url).pathname,'/v1/projects');assert.equal(new URL(requests[1]!.url).pathname,'/v1/projects');assert.equal(requests[0]?.headers.get('x-csrf-token'),null);assert.equal(requests[1]?.headers.get('x-csrf-token'),'csrf');assert.deepEqual(JSON.parse(await requests[1]!.text()),{action:'projects.create',params:{team_id:'team',project_id:'p',name:'Project'}})})
