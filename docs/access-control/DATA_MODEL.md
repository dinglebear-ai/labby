...
The implemented Project Loadout compatibility mutation accepts a caller-validated canonical Loadout name and assigns it only to an active Project for which the exact active `VerifiedIdentity` has an active direct membership carrying `project.manage`. The AccessStore does not inspect gateway configuration or infer a default Loadout. The application composition boundary must validate the name against desired gateway configuration through `GatewayManager::loadout_get` before calling the mutation. An absent mapping is inserted in one immediate transaction with the global revision, owning Organization policy epoch, Project policy epoch, and a redacted audit event. Repeating the exact mapping is a zero-write `AlreadyApplied`; attempting to replace it with a different name fails with a typed conflict. Inaccessible and cross-Organization Projects remain non-enumerating failures.
