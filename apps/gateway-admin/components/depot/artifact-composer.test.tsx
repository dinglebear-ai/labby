...
test('writing tips toggle reclaims editor width without resetting the draft', async () => {
  ...
  try {
    ...
    editor.scrollTop = 42
    await act(async () => editor.dispatchEvent(new window.Event('scroll', { bubbles: true })))
    ...
    await act(async () => source.dispatchEvent(new window.MouseEvent('click', { bubbles: true })))
    ...
    await act(async () => workspaceButtons[1].dispatchEvent(new window.MouseEvent('click', { bubbles: true })))
    ...
    await act(async () => workspaceButtons[0].dispatchEvent(new window.MouseEvent('click', { bubbles: true })))
