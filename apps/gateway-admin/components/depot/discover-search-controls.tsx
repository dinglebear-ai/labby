...
export function DiscoverSearchControls({ query, onQuery, providers, artifacts, kind, selectedProvider, onFilter, visibility = 'all', onVisibility }: {
  query: string
  ...
  const filterCount = Number(kind !== 'all') + Number(selectedProvider !== 'all') + Number(visibility !== 'all')
  return <div className="px-6 py-3.5 sm:pl-[82px]">
    <div data-discover-search="1" className="flex h-[42px] min-w-0 items-center gap-[9px] rounded-xl border border-aurora-border-strong bg-aurora-control-surface px-[13px] transition-shadow focus-within:border-aurora-accent-primary/70 focus-within:shadow-[0_0_0_1px_color-mix(in_srgb,var(--aurora-accent-primary)_45%,transparent),0_0_18px_3px_color-mix(in_srgb,var(--aurora-accent-primary)_22%,transparent)]">
      ...
      <Input name="artifact-search" aria-label="Search Labby artifacts" value={query} onChange={event => onQuery(event.target.value)} placeholder="Search artifacts across your sources" className="h-10 min-w-0 flex-1 border-0 bg-transparent px-0 text-sm font-medium shadow-none focus-visible:ring-0" />
      {providers.length > 0 ? <><span aria-hidden className="h-[18px] w-px shrink-0 bg-aurora-border-default/70 max-sm:hidden" /><div role="group" aria-label="Labby sources" className="aurora-scrollbar flex max-w-[42%] shrink-0 items-center gap-[3px] overflow-x-auto max-sm:hidden">
        {providers.map(provider => <Button data-visible-label key={provider.id} variant="ghost" size="icon-sm" disabled={!provider.enabled} aria-label={`Filter to ${provider.name.replace(/\bDepot\b/gi, 'Labby')}`} title={`${provider.name.replace(/\bDepot\b/gi, 'Labby')} — filter results`} aria-pressed={selectedProvider === provider.id} onClick={() => onFilter('provider', selectedProvider === provider.id ? 'all' : provider.id)} className="size-[26px] shrink-0 rounded-[7px] border p-0" style={{ color: 'var(--aurora-accent-strong)', borderColor: selectedProvider === provider.id ? 'color-mix(in srgb, var(--aurora-accent-primary) 55%, transparent)' : 'transparent', background: selectedProvider === provider.id ? 'color-mix(in srgb, var(--aurora-accent-primary) 10%, transparent)' : 'transparent' }}><ProviderMark origin={origins.get(provider.id)} /></Button>)}
      </div></> : null}
      ...
      <Popover>
        ...
        <PopoverContent align="end" aria-label="Kind and source filters" className="aurora-scrollbar max-h-[min(32rem,70svh)] w-[min(580px,calc(100vw-2rem))] space-y-4 overflow-y-auto rounded-xl border-aurora-border-strong bg-aurora-panel-strong p-4 text-aurora-text-primary">
          ...
          <fieldset className="space-y-2"><legend className="text-[9.5px] font-bold uppercase tracking-[0.13em] text-aurora-text-muted">Source</legend><div className="flex flex-wrap gap-1.5">{[{ id: 'all', name: 'All sources', enabled: true }, ...providers].map(provider => <Button data-visible-label key={provider.id} size="sm" className="h-[26px] rounded-lg px-2.5 text-[10.5px] font-[650]" variant={selectedProvider === provider.id ? 'secondary' : 'ghost'} disabled={!provider.enabled} aria-pressed={selectedProvider === provider.id} onClick={() => onFilter('provider', provider.id)}><ProviderMark origin={origins.get(provider.id)} />{provider.name.replace(/\bDepot\b/gi, 'Labby')}</Button>)}</div></fieldset>
          {query.trim().length >= 3 && onVisibility ? <fieldset className="space-y-2 border-t border-aurora-border-subtle pt-3"><legend className="text-[9.5px] font-bold uppercase tracking-[0.13em] text-aurora-text-muted">Result access</legend><p className="text-[10px] leading-relaxed text-aurora-text-muted">Filter the loaded search results using visibility reported by each source.</p><div className="grid grid-cols-3 gap-1.5">{([['all','All',Network],['public','Public',Globe],['team','Team',Users]] as const).map(([value,label,Icon]) => <Button data-visible-label key={value} size="sm" variant={visibility === value ? 'secondary' : 'ghost'} className="h-8 rounded-lg text-[11px]" aria-pressed={visibility === value} onClick={() => onVisibility(value)}><Icon className="size-3.5" />{label}</Button>)}</div></fieldset> : null}
          <p className="text-[10px] text-aurora-text-muted">Kinds filter the full catalog before pagination.</p>
        </PopoverContent>
      </Popover>
    </div>
    {filterCount > 0 ? <div className="flex flex-wrap items-center gap-[7px] pt-2"><span className="text-[9.5px] font-bold uppercase tracking-[0.13em] text-aurora-text-muted">Applied</span>{([{ field: 'kind', label: kind }, { field: 'provider', label: providers.find(provider => provider.id === selectedProvider)?.name ?? selectedProvider }] as const).filter(filter => filter.field === 'kind' ? kind !== 'all' : selectedProvider !== 'all').map(filter => <span key={filter.field} className="inline-flex h-[25px] max-w-[320px] items-center gap-[7px] rounded-full border border-aurora-accent-pink/35 bg-aurora-accent-pink/10 pl-2.5 pr-1.5 text-[11px] font-[650]"><span className="truncate">{filter.label}</span><Button data-visible-label variant="ghost" size="icon-sm" aria-label={`Remove ${filter.field} filter`} onClick={() => onFilter(filter.field, 'all')} className="size-4 rounded-full p-0"><X className="size-2.5" /></Button></span>)}{visibility !== 'all' ? <span className="inline-flex h-[25px] items-center gap-[7px] rounded-full border border-aurora-accent-primary/35 bg-aurora-accent-primary/10 pl-2.5 pr-1.5 text-[11px] font-[650]"><span className="capitalize">{visibility}</span><Button variant="ghost" size="icon-sm" aria-label="Remove visibility filter" onClick={() => onVisibility?.('all')} className="size-4 rounded-full p-0"><X className="size-2.5" /></Button></span> : null}</div> : null}
  </div>
}
