...
- **`code` MCP tool names** — Before: `codemode.examplemovies.movieSearch(...)`. After: `codemode.examplemovies.movie_search(...)`. Matches Cloudflare normalization so LLMs trained on their examples call the right helper.
