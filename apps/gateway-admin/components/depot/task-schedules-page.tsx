...
type PinnedAgent = AgentView & { loadout_digest?: string; image_digest?: string; harness_digest?: string; harness_id?: string | null; repository_digest?: string }
...
export function TaskScheduleRows({ rows, agents, states, busy, canOperate, canDelete, onToggle, onRun, onEdit, onDelete }: { rows: TaskSchedule[]; agents: AgentView[]; states: Record<string, TaskView>; busy?: string; canOperate: boolean; canDelete: boolean; onToggle: (row: TaskSchedule) => void; onRun: (row: TaskSchedule) => void; onEdit: (row: TaskSchedule) => void; onDelete: (row: TaskSchedule) => void }) {
  ...
  return <section aria-label="Scheduled tasks" className={PANEL}>
    ...
    <div className={`${ROW} border-b border-aurora-border-subtle !py-2 ${LABEL}`} aria-hidden="true"><span className="w-[42px] shrink-0">On</span><span className="min-w-[96px] flex-1">Task</span><span className="hidden w-[132px] shrink-0 sm:block">Schedule</span><span className="hidden w-[150px] shrink-0 min-[1180px]:block">Loadout</span><span className="hidden w-[96px] shrink-0 min-[1000px]:block">Last run</span><span className="w-[86px] shrink-0 text-right">Next</span><span className="w-[51px] shrink-0" /></div>
    {shown.map(row => {
      const agent = agents.find(value => value.agent_id === row.agent_id) as PinnedAgent | undefined
      ...
      return <article key={row.schedule_id} className="border-t border-aurora-border-subtle first:border-t-0 even:bg-aurora-control-surface/40">
        <div className={`${ROW} hover:bg-aurora-hover-bg`}>
          ...
          <span className="hidden w-[150px] shrink-0 min-[1180px]:block"><span title={agent?.loadout_digest ?? 'The server did not report a pinned loadout.'} className="block truncate rounded-full border border-aurora-accent-primary/20 bg-aurora-accent-primary/10 px-2 py-0.5 font-mono text-[9.5px] text-aurora-accent-strong">{agent?.loadout_digest ?? 'Not reported'}</span></span>
        ...
        {expanded ? <div className="grid gap-[14px] border-t border-aurora-border-subtle bg-aurora-page-bg/25 px-4 pb-4 pt-3 lg:grid-cols-[minmax(0,1.3fr)_minmax(220px,.7fr)] lg:pl-[68px]"><div><h3 className={`${LABEL} mb-2`}>Latest run</h3>{row.last_task_id ? <TaskRunDetails key={row.last_task_id} taskId={row.last_task_id} known={states[row.last_task_id]} /> : <p className="text-xs text-aurora-text-muted">No admitted runs yet.</p>}{row.last_error_kind ? <p className="mt-2 text-xs text-aurora-error">Admission failed: {row.last_error_kind}</p> : null}<p className="mt-3 text-[11px] leading-relaxed text-aurora-text-muted">Missed runs coalesce into one. Retries use fresh access checks and never replay cancelled tasks. Already admitted tasks have their own lifecycle.</p></div><div><h3 className={`${LABEL} mb-2`}>Definition</h3><Definition fields={[[ 'Schedule', `${scheduleLabel(row.schedule)} · ${scheduleTimezone(row.schedule)}` ], ['Agent', row.agent_id], ['Loadout', agent?.loadout_digest], ['Image', agent?.image_digest], ['Harness', agent?.harness_id ?? agent?.harness_digest], ['Repository', agent?.repository_digest], ['Retries', row.retry_policy?.max_retries ? `${row.retry_policy.max_retries} retries · ${row.retry_policy.backoff_ms / 60000}m delay` : 'No automatic retries'], ['Next retry', row.next_retry_at ? new Date(row.next_retry_at).toLocaleString() : 'None pending'], ['Revision', row.revision]]} /><div className="mt-2 flex justify-end gap-2"><Button data-visible-label size="sm" variant="outline" disabled={!canOperate || Boolean(busy)} onClick={() => onEdit(row)}><Pencil />Edit</Button><Button data-visible-label size="sm" variant="outline" disabled={!canDelete || Boolean(busy)} onClick={() => onDelete(row)}><Trash2 />Delete</Button></div></div></div> : null}
      </article>
    })}
