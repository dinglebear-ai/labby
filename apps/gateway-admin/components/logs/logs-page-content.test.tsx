...
test('log stream retains pause and filter controls with accessible literal row details', async () => {
  ...
  try {
    ...
    await act(async () => { pause.dispatchEvent(new window.MouseEvent('click', { bubbles: true })) })
    ...
    await act(async () => { errorFilter.dispatchEvent(new window.MouseEvent('click', { bubbles: true })) })
