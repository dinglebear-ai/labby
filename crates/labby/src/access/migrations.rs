...
pub(super) const SCHEMA_FINGERPRINT: &str = "labby-access-v8-20260913";
...
fn validate_v7_before_migration(connection: &Connection) -> AccessStoreResult<()> {
    ...
    if schema_manifest(connection)? != schema_manifest(&canonical_v7_schema()?)? {
        return Err(AccessStoreError::IntegrityViolation {
...
fn install_v8_expansion(connection: &Connection) -> AccessStoreResult<()> {
    connection
        .execute_batch(EXECUTION_EVIDENCE_SCHEMA)
        .map_err(super::store::map_sqlite_error)?;
    connection
...
mod credential_migration_tests {
    ...
    #[test]
    fn v7_execution_migration_preserves_sessions_and_enforces_bounded_evidence() {
        ...
        connection.execute("INSERT INTO agent_session_evidence VALUES('session-a','sha256:input','hello',NULL,'retained',NULL,NULL,101,102)", []).unwrap();
        assert!(
            connection
                .execute(
                    "UPDATE agent_session_evidence SET transcript=?1",
                    ["x".repeat(1_048_577)]
                )
                .is_err()
        );
        assert_eq!(
            connection
                .query_row("SELECT transcript FROM agent_session_evidence", [], |row| {
                    row.get::<_, String>(0)
                })
                .unwrap(),
            "retained"
        ...
        assert_eq!(
            connection
                .query_row("SELECT count(*) FROM agent_session_evidence", [], |row| row
                    .get::<_, i64>(0))
                .unwrap(),
            1
        );
    ...
    fn v7_execution_migration_rejects_tampering_without_changing_version() {
        ...
        assert_eq!(
            connection
                .query_row(
                    "SELECT count(*) FROM sqlite_schema WHERE name='agent_session_evidence'",
                    [],
                    |row| row.get::<_, i64>(0)
                )
                .unwrap(),
