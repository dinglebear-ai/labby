...
// Loadout · Auth · Endpoint · Scope · expand. Per-loadout call metrics are not collected on this tree, so no column claims them.
export const LOADOUT_PROFILE_COLUMNS = 'minmax(180px,1fr) 100px minmax(200px,1.2fr) 150px 64px'
...
export function LoadoutProfileHeader() {
  return <div aria-hidden="true" className="grid h-[34px] items-center gap-2 border-b border-aurora-border-strong bg-[var(--gw0-0_48)] px-4 text-[10px] font-bold uppercase tracking-[.14em] text-aurora-text-muted" style={{ gridTemplateColumns: LOADOUT_PROFILE_COLUMNS }}>
    {['Loadout', 'Auth', 'Endpoint', 'Scope', ''].map((label, index) => <span key={index} className={index > 3 ? 'text-center' : undefined}>{label}</span>)}
  </div>
}
