...
The current registered services are `artifacts`, `bundles`, `doctor`, `fs`, `gateway`, `jobs`, `lab_admin`, `server_logs`, `setup`, `snippets`, `sources`, and `uploads`. The direct stdio MCP proxy is a CLI product surface backed by the gateway runtime. [docs/services/STASH.md](docs/services/STASH.md) is an approved contract pending implementation; do not list File Stash as registered or usable until implementation and end-to-end qualification promote it into the generated catalogs.
