...
test('inspector renders revision file counts and retains the exact artifact import action', async () => {
  ...
  try {
    ...
    // Import stays disabled without an exact revision and while an import is in flight.
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={{ ...artifact, currentRevision: { fileCount: 0 } }} />)
    const noRevision = [...document.querySelectorAll('button')].find(node => node.textContent === 'Add to Library')
    assert.ok(noRevision)
    assert.equal(noRevision.disabled, true)
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={artifact} importing />)
    const adding = [...document.querySelectorAll('button')].find(node => node.textContent === 'Adding…')
    assert.ok(adding)
    assert.equal(adding.disabled, true)
    assert.equal(adding.getAttribute('title'), 'Adding…')
    await view.rerender(<DiscoverArtifactInspection {...props} artifact={artifact} />)
