...
test('direct route manifest fails closed for unknown routes and gates known ones', () => {
  ...
  assert.equal(capabilityForPath('/people'), 'membership.manage')
...
test('workspace routes stay reachable for a plain member and admin routes do not', () => {
  ...
  for (const route of ['/administration', '/people', '/settings', '/logs', '/browsers']) assert.ok(!reachable.includes(route), `${route} must not be reachable for a member`)
})
...
test('control-plane navigation excludes the redundant Labby settings shortcut', () => {
  ...
  assert.deepEqual(controlPlane?.items.map((item) => item.id), ['Overview', 'Gateway', 'People', 'Browsers'])
