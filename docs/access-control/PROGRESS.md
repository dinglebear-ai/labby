...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. Implementation includes separate unmounted, generation-bound manager compositions for runtime Loadout/upstream-tool publication and runtime Loadout/virtual-server-filtered built-in MCP service/action publication. These are not yet composed with the stable Project runtime context or into one complete gateway catalog or dispatch grant, and access-control enforcement is not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
...
- [ ] exact Project + policy epoch/complete catalog generation cache key. Waves 16 and 18 separately publish generation-bound runtime Loadout/upstream-tool facts and Loadout/virtual-server-filtered built-in service/action facts, but neither is yet composed with the stable Project context, subject OAuth catalogs, other MCP capability families, exact targets, or the final dispatch boundary.
