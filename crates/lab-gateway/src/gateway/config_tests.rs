use lab_runtime::gateway_config::{GatewayConfig, ProtectedMcpRouteConfig, UpstreamConfig};
...
fn sample_import_source() -> lab_runtime::gateway_config::ImportSource {
    lab_runtime::gateway_config::ImportSource::new(
        "codex",
...
fn insert_upstream_clears_import_tombstone_by_source_identity_after_rename() {
    let mut cfg = sample_config();
    let source = lab_runtime::gateway_config::ImportSource::new(
        "codex",
        "/home/alice/.codex/config.toml",
        "2026-05-15T00:00:00Z",
    )
