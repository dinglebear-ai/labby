...
When disabled, Lab filters built-in upstream API services that talk to external services, including ExampleMovies, ExampleSeries, ExampleIndexer, ExampleMedia, ExampleMetrics, ExampleUsenet, exampledownload, Tailscale, Linkding, Memos, Bytestash, Paperless, Arcane, Unraid, UniFi, ExampleRequests, Gotify, OpenAI, Qdrant, TEI, Apprise, MCP Registry, AdGuard, Pi-hole, Dozzle, Glances, ExampleStream, Komga, NPM, Uptime Kuma, Neo4j, NotebookLM, OpenACP, ExampleSuite, and other non-bootstrap registry entries.
...
Runtime smoke verified that disabling built-in upstream APIs rejects an upstream service such as ExampleMovies while preserving bootstrap services and preserving TOML comments/unknown tables.
