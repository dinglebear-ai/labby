...
- **Brand-colored icon backgrounds:** every service icon box uses the service's brand color (e.g., ExampleMovies gold, ExampleSeries sky-blue, ExampleMedia orange) with the white variant of the logo on top. For services not in Simple Icons (Apprise, Arcane, ByteStash, Gotify, Linkding, Memos, TEI), a white inline SVG renders on the brand background. No external favicon APIs.
...
Apprise, Arcane, ByteStash, Gotify, Linkding, Memos, OpenAI, ExampleRequests, Paperless, ExampleMedia, ExampleIndexer, exampledownload, Qdrant, ExampleMovies, ExampleUsenet, ExampleSeries, Tailscale, ExampleMetrics, TEI, UniFi, Unraid.
...
- Apply populates the Custom form: Name = detected service key (e.g. `examplemovies`), Transport = `http`, URL = value of `{SERVICE}_URL` env var if present. Closes the drawer on apply.

**Service detection:** match env var prefixes (`EXAMPLEMOVIES_`, `EXAMPLESERIES_`, etc.) against the 21 known services.
