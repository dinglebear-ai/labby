...
ADR 0006 supersedes the previous prohibition on source-IP telemetry. Network provenance, including the direct socket peer and a trusted-proxy-resolved effective client IP, is part of Labby's accepted target observability contract. The current UsageStore and Traces implementation does not yet populate those fields universally, so a missing address must not be interpreted as an anonymous or network-less caller. Forwarded client-IP headers remain untrusted until the explicit trusted-proxy policy required by ADR 0006 is implemented.

Complete plaintext payload observability is also an accepted target under ADR 0006, but raw request/response envelopes and secret-bearing headers belong in the separately protected forensic capture plane defined there, not duplicated through ordinary structured log sinks. Until that plane exists, the existing redaction requirements below remain mandatory for normal logs and traces.
