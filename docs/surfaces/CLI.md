...
Contexts occupy only the `[cli]` table of the canonical host `config.toml`. Writes reuse its lock and atomic writer and preserve unrelated tables and comments. Names are bounded, URLs cannot contain credentials, query strings, or fragments, and plaintext HTTP is limited to loopback. Active contexts must be cleared or replaced before removal. Repeating the same selection is a no-op. `auth status` reports locally saved session presence and the grant's saved `granted_scopes` for the selected destination. A null `granted_scopes` value means no saved scope record is available. It does not refresh credentials or verify online authorization. These scopes belong to the Labby CLI's saved OAuth session; they do not describe a separate Codex MCP connection or its token. `auth logout` removes only that destination's saved session, not its provider account.
