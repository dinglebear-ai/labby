...
test('gateway table sorts servers by name and shows full stdio command line', () => {
  ...
  assert.match(markup, /Sort by clients[\s\S]*Sort by endpoint[\s\S]*Sort by exposed[\s\S]*Sort by uptime/)
  assert.match(markup, /Sort by uptime/)
  assert.match(markup, /aria-sort="none"[^>]*><span>Uptime<\/span>/)
  assert.match(markup, /data-gateway-column="clients"/)
  ...
  assert.doesNotMatch(markup, /Reorder .* column/)
})
