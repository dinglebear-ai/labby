...
pub(super) struct PersistedGatewayRuntimeEntry {
    ...
    #[serde(default)]
    owner: Option<crate::gateway::types::GatewayRuntimeOwnerView>,
    #[serde(default)]
        ...
        for upstream in &cfg.upstream {
            if name.is_some_and(|name| name != upstream.name)
                || scope
                    .route_visible_upstreams
                    .as_ref()
                    .is_some_and(|visible| !visible.contains(&upstream.name))
            {
                continue;
            }
            ...
            let live_stale_count = if upstream.command.is_some() {
                ...
                likely_stale_process_group_count(upstream, live_runtime, &process_matches)
            } else {
                0
            };
...
) -> crate::gateway::types::GatewayRuntimeOwnerView {
    crate::gateway::types::GatewayRuntimeOwnerView {
