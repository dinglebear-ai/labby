...
Implement and debug Lab's inline protected MCP route flow so OAuth-enabled upstream MCP servers such as syslog and Axon can be added in the Gateway UI, connected with upstream OAuth, and published through Lab OAuth at paths such as `https://mcp.tootie.tv/syslog` and `https://mcp.tootie.tv/axon`.
...
- Rebuilt and hot-swapped the running `labby` container, then verified live public protected routes through `mcp.tootie.tv`.
...
- Configured live `syslog` and `axon-tootie-tv` upstreams and protected routes in `/home/jmagar/.lab/config.toml`.
- Fixed upstream OAuth callback allowlists for syslog and Axon so Lab can complete dynamic client registration with `https://lab.tootie.tv/auth/upstream/callback`.
...
- `docker logs labby` - confirmed live startup discovery success for `syslog` and `axon-tootie-tv`.
- `curl` calls to `/v1/gateway` with bearer auth - confirmed `gateway.test` and `gateway.list` runtime views.
- `curl` calls to `https://mcp.tootie.tv/syslog` and `https://mcp.tootie.tv/axon` with Lab JWTs - confirmed public protected routes initialize successfully.
...
- Upstream OAuth dynamic registration returned redirect URI validation failures for Lab's callback. Syslog and Axon needed `https://lab.tootie.tv/auth/upstream/callback` in their upstream OAuth allowlists.
- Axon Google login returned `redirect_uri_mismatch` until its Google OAuth client/env config was aligned with `https://axon.tootie.tv/auth/google/callback`.
...
| Gateway UI/test showed no capabilities for OAuth upstreams behind protected routes. | `syslog` and `axon-tootie-tv` show connected state and discovered tools/resources. |
...
| `/v1/gateway gateway.test axon-tootie-tv` | Tools/resources discovered | `tool_count=1`, `resource_count=2`, `last_error=null` | pass |
| `https://mcp.tootie.tv/syslog initialize` | Public protected route returns upstream initialize result | HTTP 200, `serverInfo.name=syslog-mcp` | pass |
| `https://mcp.tootie.tv/axon initialize` | Public protected route returns upstream initialize result | HTTP 200 SSE, Axon/RMCP initialize payload | pass |
...
- Whether the stale `axon` protected route pointing `/lab` at `https://lab.tootie.tv/mcp` should be removed from live config.
...
- Follow-on: use browser automation to confirm the Gateway UI displays the now-discovered capabilities for `syslog` and `axon-tootie-tv`.
