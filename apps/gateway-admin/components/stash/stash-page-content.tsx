...
import { Copy, Download, File, FileArchive, FileCode2, FileJson, FileText, Grid2X2, MoreHorizontal, Image, Inbox, List, Pencil, RefreshCw, Search, Trash2, Upload, X } from 'lucide-react'
...
function fileIcon(name: string) {
  ...
  if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'svg', 'avif', 'heic'].includes(ext || '')) return <Image aria-hidden="true"/>
...
export function StashPageContent() {
  ...
  const visibleFiles = useMemo(() => files.filter((file) => kind === 'All' || stashFileKind(file.display_name) === kind).sort((a, b) => {
    const order = sort === 'name' ? a.display_name.localeCompare(b.display_name) : sort === 'kind' ? stashFileKind(a.display_name).localeCompare(stashFileKind(b.display_name)) : sort === 'size' ? a.size_bytes - b.size_bytes : a.created_at - b.created_at
    return ascending ? order : -order
  }), [files, kind, sort, ascending])
  ...
  return <>
    ...
    <button type="button" onClick={() => input.current?.click()} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = 'copy' }} onDrop={event => { event.preventDefault(); acceptFiles(event.dataTransfer.files) }} className="group flex w-full flex-wrap items-center justify-center gap-2.5 rounded-aurora-2 border-[1.5px] border-dashed border-aurora-border-strong/50 bg-transparent p-[18px] text-aurora-text-muted transition-colors hover:border-aurora-accent-primary hover:bg-aurora-accent-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-aurora-accent-primary">
      <Upload className="size-4"/><strong className="text-[12.5px] font-semibold">Drop files here or browse</strong><span className="text-[11px]">agents read this over MCP as <code>stash://</code></span>
    </button>
    ...
    <DashboardPanel title="Files" headerStyle={{ padding: '10px 15px', background: 'var(--gw0-0_38)' }} bodyStyle={{ padding: 0 }} action={<div className="flex flex-wrap items-center gap-2">
      ...
      {loading ? <div role="status" className="py-10 text-center text-sm text-aurora-text-muted">Loading your files…</div> : visibleFiles.length === 0 ? <><div className="py-10 text-center"><Inbox className="mx-auto size-8 text-aurora-text-muted"/><strong className="mt-3 block text-aurora-text-primary">{query || kind !== 'All' ? 'No matches on this page' : 'Your Stash is empty'}</strong><p className="mt-1 text-sm text-aurora-text-muted">{query || kind !== 'All' ? (nextCursor ? 'More files are available to search.' : 'Try another filename or file kind.') : 'Upload a file to make it available to your agents.'}</p></div>{nextCursor ? <Button className="mx-auto mt-3" variant="outline" disabled={loadingMore} onClick={() => void load(query.trim(), nextCursor)}>{loadingMore ? 'Loading…' : 'Load more'}</Button> : null}</> : <>{view === 'list' ? <div className="hidden grid-cols-[minmax(0,1fr)_80px_120px_70px_80px_26px] gap-3 border-b border-aurora-border-default px-4 py-2 text-[9.5px] font-bold uppercase tracking-[.13em] text-aurora-text-muted md:grid">{([['name', 'Name'], ['kind', 'Kind'], ['access', 'Access'], ['size', 'Size'], ['added', 'Added']] as const).map(([key, label]) => key === 'access' ? <span key={key}>{label}</span> : <button key={key} type="button" className="text-left uppercase" onClick={() => { if (sort === key) setAscending((value) => !value); else { setSort(key); setAscending(true) } }} aria-label={`Sort loaded files by ${label.toLowerCase()}`}>{label}{sort === key ? ascending ? ' ↑' : ' ↓' : ''}</button>)}<span/></div> : null}<div className={view === 'grid' ? 'grid gap-3 sm:grid-cols-2 xl:grid-cols-3' : 'divide-y divide-aurora-border-subtle'}>{visibleFiles.map(file => <FileRow key={file.file_id} file={file} grid={view === 'grid'} onDelete={() => setDeleteTarget(file)} onManage={() => setManageTarget(file)} onDownload={() => download(file)} onCopy={async () => { const result = await copyUri(value => navigator.clipboard.writeText(value), file.uri); if (result.ok) setAnnouncement(result.announcement); else setError(result.error) }}/>)}</div>{nextCursor ? <Button className="mx-auto mt-3" variant="outline" disabled={loadingMore} onClick={() => void load(query.trim(), nextCursor)}>{loadingMore ? 'Loading…' : 'Load more'}</Button> : null}</>}
    </DashboardPanel>
...
function FileRow({ file, grid, onDelete, onManage, onDownload, onCopy }: { file: StashFile; grid: boolean; onDelete: () => void; onManage: () => void; onDownload: () => Promise<void>; onCopy: () => Promise<void> }) {
  ...
  return <article className={grid ? 'group rounded-aurora-2 border border-aurora-border-subtle bg-aurora-panel-low p-4' : 'group grid grid-cols-[minmax(0,1fr)_26px] items-center gap-3 border-b border-aurora-border-subtle px-4 py-2.5 last:border-b-0 hover:bg-aurora-hover-bg md:grid-cols-[minmax(0,1fr)_80px_120px_70px_80px_26px]'}>
    ...
    <span title="File kind inferred from filename extension" className={`${grid ? 'mt-3 block' : 'hidden md:block'} text-[10.5px] text-aurora-text-muted`}>{stashFileKind(file.display_name)}</span>
    <span className={`${grid ? 'mt-2 block' : 'hidden md:block'} text-[10.5px] font-semibold ${file.owned ? 'text-aurora-text-muted' : 'text-aurora-success'}`}>{file.owned ? 'Owner' : 'Shared with you'}</span>
    <span className={`${grid ? 'mt-2 block' : 'hidden md:block'} text-[10.5px] tabular-nums text-aurora-text-muted`}>{bytes(file.size_bytes)}</span>
    <time dateTime={added.toISOString()} title={added.toLocaleString()} className={`${grid ? 'mt-2 block' : 'hidden md:block'} text-[10.5px] tabular-nums text-aurora-text-muted`}>{added.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</time>
