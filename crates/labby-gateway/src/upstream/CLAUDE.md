...
| `pool/relay.rs` | `RelayClientHandler` — a `ClientHandler` for *dedicated* upstream connections that forwards server→client requests (elicitation/sampling/roots) down to the agent's `Peer<RoleServer>`, advertising only the capabilities the agent itself declared. `UpstreamPool::call_tool_relayed(config, subject, params, downstream, session_id)` returns a relay connection from a per-`(upstream, session_id, subject)` cache (`relay_connections`) — TTL + dead-transport eviction, LRU cap, single-flight, swept by the shared sweep loop — or opens one via the generic `connect::connect_upstream_with_handler` seam. The cache key has three parts: `session_id` (minted per `LabMcpServer` session) binds a cached connection to one downstream agent so it is never reused across agents; `subject` (the OAuth identity, `None` on the raw path) prevents a connection authenticated as one identity from serving another within a session. Relayed calls use the pool's `relay_timeout` (default 5 min, `upstream_relay_timeout_ms`) — **not** the 30s `request_timeout` — because they block on a human answering the elicitation. **Scope: `call_tool` only** — `read_resource`/`get_prompt` still use the unit handler and decline upstream server→client requests. Wired into both MCP proxy branches (`mcp/call_tool_upstream.rs`) behind the `LABBY_UPSTREAM_RELAY_ELICITATION` env gate + a downstream-elicitation-capability check. |
