...
This makes persistent disabled state win by default. `--services examplemovies` should fail as unknown when upstream APIs are disabled.
...
fn upstream_api_filter_removes_upstreams_and_keeps_bootstrap() {
    ...
    for removed in ["examplemovies", "exampleseries", "tailscale", "openai"] {
...
fn services_allowlist_does_not_reenable_globally_disabled_upstreams() {
    ...
    let error = super::filter_registry(reg, &["examplemovies".to_string()])
        .expect_err("disabled examplemovies should be unknown to --services");
...
```json
{
  "services": {
    "built_in_upstream_apis_enabled": true,
    "built_in_upstream_api_services": ["examplemovies", "exampleseries", "openai"],
