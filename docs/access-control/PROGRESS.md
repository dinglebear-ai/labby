...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. GatewayManager combines the generation-bound Loadout-filtered upstream-tool and built-in MCP service/action projections over one bounded common publication interval, and separately publishes an exact-generation Project-bound protected-route narrowing snapshot. These observations are not yet composed into a server-owned request/session binding or dispatch grant, and access-control enforcement is not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
