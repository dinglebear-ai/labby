...
`examplemovies`, `exampleseries`, `exampleindexer`, `examplerequests`, `examplemetrics`, `arcane`, `examplemedia`, `exampleusenet`, `exampledownload`, `unifi`, `qdrant`, `tei`, `tailscale`, `apprise`, `gotify`, `bytestash`, `linkding`, `memos`, `openai`, `paperless`, `unraid`, `extract`, `device_runtime`.
...
Run `cargo run -p lab --all-features -- audit onboarding examplemovies mcpregistry --json` and confirm the new metadata checks pass. If unrelated onboarding checks fail, record that separately and rely on the targeted metadata script for bead closure.
