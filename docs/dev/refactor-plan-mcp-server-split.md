...
| `context.rs` | request-context inherent methods (`request_subject*`, `request_actor_key`, `request_runtime_owner`, `code_mode_surface`, `oauth_upstream_config(s)`) + free fns (`redact_subject_for_logging`, `*_from_extensions`, `oauth_upstream_subject_for_request`, scope checks: `code_mode_search_scope_allowed`, `tool_execute_scope_allowed`, `tool_execute_builtin_action_allowed`, `builtin_action_requires_admin`) | 2160, 2165–2217, 2324–2395 | ~250 |

**Visibility note (Revision 2 — finding #3).** The four scope/admin gate fns — `code_mode_search_scope_allowed`, `tool_execute_scope_allowed`, `tool_execute_builtin_action_allowed`, `builtin_action_requires_admin` (private `fn`s today at `server.rs:2352–2395`) — must be **widened to `pub(crate)`** in `context.rs` so `call_tool.rs` / `call_tool_codemode.rs` can call them. This is a **visibility change only, no logic change**: the auth surface is unchanged (same-crate widening). Their tests (`gateway_builtin_actions_require_admin_scope`, `code_mode_scope_allows_read_but_tool_execute_does_not`, `setup_destructive_builtin_actions_require_admin_scope`) must stay green in `context/tests.rs`. A reviewer should see only the `pub(crate)` keyword move with the function body byte-identical.
...
async fn call_code_mode_impl(&self, service: &str, args: &JsonObject, context: &RequestContext<RoleServer>)
...
1. **`search` scope+dispatch** (`service == CODE_MODE_SEARCH_TOOL_NAME`) → `call_code_mode_impl`. Scope gate `code_mode_search_scope_allowed` runs **inside** this branch and self-returns on denial. **Self-returns.**
...
3. **MEDIUM — `context.rs` scope-fn visibility note.** Added to the §3 `context.rs` row: `code_mode_search_scope_allowed`, `tool_execute_scope_allowed`, `tool_execute_builtin_action_allowed`, `builtin_action_requires_admin` are **widened to `pub(crate)`, no logic change**; their tests stay green in `context/tests.rs`.
