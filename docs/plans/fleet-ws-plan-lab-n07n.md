...
source = "controller"
target = "node-b"
actions = ["examplemovies.queue.list", "exampleseries.*"]
...
### Additional tests (append to Testing)
- [ ] Confused-deputy: policy allows `(controller → node-b, examplemovies.*)` with `allow_destructive=false`; invoking `examplemovies.movie.delete` returns `denied` with `reason: destructive_not_permitted`.
