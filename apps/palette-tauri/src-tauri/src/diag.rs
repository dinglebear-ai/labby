...
//! Labby config dir (`~/.labby/logs/palette.log`, overridable via
//! `LABBY_PALETTE_LOG_PATH`). Append-only; warnings are infrequent so no rotation.

use std::io::Write as _;
use std::path::{Path, PathBuf};

fn log_path() -> Option<PathBuf> {
    std::env::var_os("LABBY_PALETTE_LOG_PATH")
        .map(PathBuf::from)
        .or_else(|| dirs::home_dir().map(|home| home.join(".labby/logs/palette.log")))
}
