...
fn parse_rejects_lab_action_id() {
    let err = CodeModeToolId::parse("lab::examplemovies.movie.search")
...
async fn schema_rejects_lab_action_id() {
    ...
    let err = broker
        .schema("lab::examplemovies.movie.search", CodeModeCaller::TrustedLocal, CodeModeSurface::Cli)
        .await
...
async fn code_execute_callTool_lab_id_returns_unknown_tool() {
    ...
    let response = broker
        .execute(
            r#"await callTool("lab::examplemovies.movie.search", {query:"Matrix"})"#,
...
const LAB_ACTION_UNKNOWN_TOOL_HINT: &str =
    "Code Mode handles upstream MCP tools only. For Lab actions, use the `tool_execute` MCP tool: \
     name=<service> (e.g. \"examplemovies\"), arguments={action: \"<dotted.action>\", params: {...}}. \
     Example: tool_execute(name=\"examplemovies\", arguments={action:\"movie.search\", params:{query:\"Matrix\"}}).";
```
...
assert!(message.contains("\"examplemovies\""));
```
...
Run:
```bash
LAB_MCP_HTTP_TOKEN=$(...) mcporter call lab-prod.invoke name=code_schema arguments:='{"id":"lab::examplemovies.movie.search"}' 2>&1
```
