...
def classify(event: str, paths: list[str]) -> dict[str, bool]:
    ...
    rust = any_match(
        paths,
        lambda p: starts(p, "crates/", "tests/", ".cargo/")
        or p
        in {
            "Cargo.toml",
            "Cargo.lock",
    ...
    docker = rust or web or any_match(
        paths,
        lambda p: starts(p, "config/")
        or p in {".dockerignore", "docker-compose.yml", "docker-compose.prod.yml"},
    )
    release = rust or web or docker or any_match(
        paths,
        lambda p: p in {"CHANGELOG.md", "README.md", "scripts/install.sh", "scripts/install.ps1"}
        or starts(p, "plugins/labby/", "plugins/scripts/"),
    )
    security = any_match(
        paths,
        lambda p: p in {"Cargo.lock", "deny.toml", ".gitguardian.yaml"} or starts(p, ".cargo/"),
    )
