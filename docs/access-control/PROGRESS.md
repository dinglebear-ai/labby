...
- [x] **Wave 48 / Milestone 0AZ incarnation-bound exact regular Tool call kernel:** add an unmounted pool-local prepare/apply kernel with exact Tool publication and connection-incarnation checks, one queue-plus-RPC deadline, full bounded response/error preservation, and checked health attribution. Destructive/admin/AssetUse authorization, SEP-2243 recovery, usage telemetry, manager/handler/Project mounting, OAuth, and relay remain deferred. Tracked under `lab-ub0sa`.
- [x] **Wave 49 / Milestone 0BA exact regular Tool usage attribution:** schedule existing bounded best-effort usage recording only inside successful exact Tool publication/incarnation checked apply. Stale, queued, and pre-apply-cancelled outcomes remain unrecorded; SEP-2243 recovery and mounting remain deferred. Tracked under `lab-9mfij`.
- [x] **Wave 50 / Milestone 0BB exact Tool SEP-2243 recovery:** add one bounded header-cache refresh and retry under the exact Tool kernel's original permit/deadline, with exact read-only rechecks before refresh/retry and final checked attribution. The peer cache remains a non-authoritative hint; mounting and authorization remain deferred. Tracked under `lab-laq8e`.
- [x] **Wave 51 / Milestone 0BC AssetUse exact nondestructive Tool seam:** uniquely bind raw regular Tool names to a fresh AssetUse Access/manager interval, reject dynamic built-in/permanent/destructive collisions, execute through the exact manager pool lease, rebind before exposure, and sanitize structured MCP failures at the product boundary. Handler/transport/admin/destructive/task/OAuth/relay mounting remains deferred. Tracked under `lab-68a97`.
- [x] **Wave 52 / Milestone 0BD transport-bound exact Tool wrapper:** validate the immutable middleware-owned token expiry and identity before and after Wave 51 execution, derive route facts only from the bound core, and remain unmounted pending either relay/task lifecycle authority or an explicitly Complete-only terminal adapter. Tracked under `lab-0k1wm`.
- [x] **Wave 53 / Milestone 0BE Complete-only protected Tool terminal adapter:** preserve Complete responses above Wave 52 while dropping any non-Complete success payload that reaches the adapter into one static redacted terminal class with no retry, fallback, relay, or task registration. Current rmcp capability/protocol rejections remain sanitized MCP errors rather than being reclassified from spoofable codes or messages. Handler mounting and relay/task lifecycle remain deferred. Tracked under `lab-top1o`.
- [x] **Wave 54 / Milestone 0BF protected Complete-only Tool handler mount:** branch on the middleware Project observation before destructive/scope dispatch; retain Legacy behavior, terminate Unavailable, and route Bound calls only through Wave 53 with no fallback. Exact regular names require AssetUse; published built-ins and synthetics remain discoverable but fail closed at execution until an exact AssetUse seam lands. The mounted branch inherits the lower seam's exact-name, permission, expiry, ABA, metadata, cancellation, and zero-RPC exclusions while remaining outside OAuth, relay, widget, elicitation, and task lifecycles. Tracked under `lab-3pyb6`.
- [x] **Wave 55 / Milestone 0BG Project-bound `tools/list` enforcement:** preserve Legacy listing exactly, make Unavailable empty and non-enumerating, and filter Bound built-ins, regular upstream rows, and centrally owned synthetics from the same immutable transport publication before collision, hash, and pagination. Built-in descriptors require exact published service description and action metadata; Bound Code Mode remains available only when published but intentionally carries no mutable desired-config upstream hints. PeerContract applies the identical policy; Bound cursors include credential, route, Access, and catalog publication identity so expiry and ABA fail closed. Published/allowed destructive rows remain discoverable while Wave 54 call execution continues to reject them pending a separate confirmation/admin contract. Tracked under `lab-08f5k`.
...
- [x] exact final dispatch authorization for protected regular Prompt reads, Resource reads, and nondestructive Complete-only Tool calls. Other actions, built-ins, destructive Tools, OAuth/relay, and task/elicitation lifecycles remain deferred.
...
- [x] MCP `tools/list` filtering for protected Project routes; other discovery families remain deferred.
- [x] MCP direct authorization for protected regular Prompt, Resource, and nondestructive Complete-only Tool invocation; other capability families remain deferred.
