...
| Optional microVM | **Implemented, opt in on Linux.** Restricted Microsandbox guest; no network; cached image only (`--pull never`); 1 CPU; 256 MiB; lifecycle tied to the pooled runner. Explicit cleanup is bounded and failures fall back to a separately bounded best-effort removal. A 24-hour guest backstop covers ungraceful host death, and live pools proactively recycle before it. Only the validated Labby executable is mounted read-only. MCP authority and secrets remain in the host. |
