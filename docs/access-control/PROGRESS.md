...
The design packet and initial authentication/domain/persistence foundation are implemented in this worktree. The AccessStore now has explicit owner bootstrap, an authenticated browser-only bootstrap endpoint, and read-only doctor/setup health projection. Implementation has reached the unmounted publication prerequisites for coherent Access, runtime Loadout, upstream tool-catalog, and runtime-pool observations. Manager composition of those facts and access-control enforcement are not implemented, so existing product behavior is not authorization-gated by this work. Startup bootstrap, personal-Labby pairing, and transfer protocols also remain unimplemented.
...
- [x] **Wave 15 / Milestone 0S published runtime-pool identity prerequisite:** `GatewayRuntimeHandle` publishes its optional current `UpstreamPool` with an opaque process-local identity in one immutable state. Every swap advances the identity, including identical pool, repeated `None`, and A-B-A publications; clones share a stream and distinct handles do not collide. This enables later manager composition of published runtime Loadout and pool tool-catalog facts, but that composition and all enforcement remain unimplemented.
...
- [ ] exact Project + policy epoch/complete catalog generation cache key. Waves 14 and 15 supply the upstream tool/routability snapshot and coherent runtime-pool identity prerequisites, not manager composition or a complete gateway catalog generation.
