...
register_service!(
    ...
    "retired-upstream",
    retired-upstream,
    actions = crate::dispatch::retired-upstream::ACTIONS,
    dispatch = dispatch_fn!(crate::dispatch::retired-upstream::dispatch)
);
```
...
```text
retired-upstream: test-only
retired-upstream: test-only
retired-upstream: live wrapper only
linkding: test-only
...
- `retired-upstream` is the easiest hybrid candidate to fully normalize because its registry already sources actions from `dispatch`.
- `retired-upstream` is not a pure const-based service; keep its `actions()` path intact unless the dispatch layer is reworked first.
