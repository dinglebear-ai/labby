...
class BootstrapFakeTest(unittest.TestCase):
    ...
    def run_bootstrap(self, work, state, fail_after=None, fail_list_column=None, dry_run=False):
        ...
        args = [str(ROOT / "scripts/incus-bootstrap.sh"),"--name","fixture","--version","v1.0.0","--local-binary",str(binary),"--storage-driver","dir","--storage-pool","pool","--no-backup-config"]
    ...
    def test_available_dry_run_plans_absent_and_stopped_containers(self):
        for customized, expected in ((False, "'incus' 'launch'"), (True, "'incus' 'start' 'fixture'")):
            with self.subTest(customized=customized), tempfile.TemporaryDirectory() as td:
                before = self.baseline(customized)
                result, after = self.run_bootstrap(
                    pathlib.Path(td), copy.deepcopy(before), dry_run=True
                )
