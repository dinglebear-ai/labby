...
- **Real duplication source = stale codex plugin cache.** `~/.codex/plugins/cache/labby-marketplace/lab/local/.mcp.json` and `~/.codex/plugins/cache/jmagar-lab/lab/0.5.3/.mcp.json` still use bare `lab mcp` (stdio), while the repo source `plugins/lab/.mcp.json` and the current Claude caches use `type: http, url: ${server_url}/mcp`. Each bare `lab mcp` loads the full `~/.lab/config.toml` and clones every stdio upstream.
