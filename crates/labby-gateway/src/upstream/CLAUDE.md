...
| `pool/connect_stdio_tests.rs` | Unix-only regressions for the stdio connect path: a child that exits before answering the handshake is spawned exactly once and is never remembered as a legacy-lifecycle command, even when its stderr tail resembles a lifecycle rejection, and a **live** child's stderr never drives that classification either. |
...
| `pool/list_changed_refresh.rs` | Per-upstream, coalesced `list_changed` refresh workers (`ListChangedRefresher`, `ListChangedKinds`, `LIST_CHANGED_COALESCE_WINDOW`). The bus consumer only records which kinds an exact upstream reported; a worker task per upstream waits one coalescing window, re-lists that upstream's tools and/or resources concurrently (`refresh_after_list_changed`; prompts need none), then calls the injected forwarder so the downstream notification is never sent while the cache is still stale. One upstream's slow re-list never delays another upstream's events. Dropping the refresher aborts its workers, so its lifetime matches the consumer that feeds it. |
| `pool/list_changed_refresh_tests.rs` | Regressions for the refresh workers: a gated slow re-list does not delay another upstream's forwarded notification, a burst from one upstream collapses to at most two re-lists with one forwarded notification each, prompts forward without a re-list, and dropping the refresher aborts in-flight workers. |
| `pool/notifications_tests.rs` | Focused regressions for acknowledgement visibility, stale-generation isolation, concurrent refresh deadlines, and retry after initial failure. Its `SubscriptionServer` fixture (with a `resources/list` gate and call counter) is shared with the list-changed refresh tests. |
...
| `pool/catalog_publication.rs` | Immutable publication of the routable tool, regular-resource, regular-resource-template, and regular-prompt projections; deliberately excludes UI resources/templates, skills, OAuth subjects, local and synthetic rows, and unrelated capability health. Also owns each upstream's retained resource snapshot (`ResourceSource`, regular and `ui://` rows validated and byte-budgeted together, stamped with `listed_at`), the cache-only listing `cached_upstream_resources_allowed`, and `cold_resource_snapshots`, which classifies snapshots as missing or stale for the warm-up. A snapshot **admission** failure (duplicate or malformed URI, oversized row, fleet retention-budget breach) is a per-upstream verdict for resources and resource templates: it withholds only that upstream from the projection and never unpublishes the fleet. The verdict is retained in `ResourceSourceState::Failed` / `ResourceTemplateSourceState::Failed` and read back by `withheld_snapshots`, which is what operator surfaces render; it is deliberately **not** written to the entry's capability `last_error`, because resources, resource templates and resource reads share that one slot and every success clears it. The other publication failures remain fleet-wide and all-or-nothing: an incarnation mismatch (an internal invariant violation), the `MAX_UPSTREAM_RESOURCES` route cap, the publication-time byte ceiling, and the whole prompt projection. Each projection carries an independent generation so one catalog family cannot perturb another's publication identity. Owns the `CatalogWriteGuard` republish invariant: `catalog_write()` hands out the guard and its `Drop` calls `publish_if_changed()`, so no caller has to mark a projection dirty by hand. A `land:` merge silently dropped this module's `mod` declaration, which is what motivated the `xtask` module-reachability gate. |
...
| `pool/resources_list.rs` | Bounded resource listing + synthetic `gateway_*` documents. Three entry points: the live fan-out (`list_upstream_resources*`, which republishes snapshots), the cache-only projection the MCP `resources/list` handler and Code Mode read (`cached_upstream_resources_with_provenance_allowed`), and the single-flight warm-up that re-lists only missing or stale snapshots (`spawn_resource_snapshot_warmup`, run to completion regardless of the caller's deadline). Both listings share one bound (`bound_listed_resources`). OAuth listings reuse the per-subject connection cache. Native `ui://` (mcp-ui) resources skip the `lab://upstream/{name}/…` rewrite so they stay addressable by the same URI a tool's `_meta.ui.resourceUri` references. |
...
- **Lifecycle classification reads the MCP error alone.** `compatibility_retry`
  must be given `StdioConnectError::protocol_error()`, never
  `diagnostics_with_error()`. The latter appends the child's captured stderr,
  and "Method not found" is ordinary MCP server log vocabulary, so matching
  protocol patterns against it downgrades and respawns healthy upstreams. The
  child-exit gate cannot cover this: a still-running child never sets it, and
  the exit is not observable at all when the handshake fails at the send step.
  Full diagnostics remain correct for operator-facing errors, the fallback log
  line, and `cache_repair`, which is legitimately stderr-driven.
