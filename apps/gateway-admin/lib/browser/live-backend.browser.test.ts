...
async function action(page: Page, csrfToken: string, service: string, name: string, params: object) {
  return page.evaluate(
    async ({ csrfToken, service, name, params }) => {
...
async function addGatewayThroughUi(page: Page, name: string) {
  ...
  const mutation = page.waitForResponse(
    (response) => response.request().method() === 'POST' && new URL(response.url()).pathname === '/v1/gateway',
    { timeout: 10_000 },
  )
...
async function stageProtectedRouteThroughUi(page: Page, name: string) {
  await page.goto('/settings/surfaces/', {
  ...
  await assert.doesNotReject(
    panel.getByText(name, { exact: true }).first().waitFor({ state: 'visible', timeout: 10_000 }),
  )
...
async function removeProtectedRouteThroughUi(page: Page, name: string) {
  ...
  const removeButton = panel.getByRole('button', {
    name: `Remove protected route ${name}`,
  })
...
test(
  'embedded Gateway Admin completes a real backend journey',
  {
    concurrency: false,
    ...
    await withAbsoluteDeadline(async (signal) => {
      ...
      await useBrowserWithAbort(
        ...
        async (browser) => {
          ...
          let evidence: ReturnType<typeof observeLivePage> | undefined
          let failure: unknown
          ...
          try {
            ...
            evidence = observeLivePage(page, descriptor.base_url)
            ...
            const duplicate = await Promise.all([
              action(page, csrfToken, 'gateway', 'gateway.add', {
                spec: {
                  name: `${ownedName}-duplicate`,
            ...
            progress(
              `requests:count=${evidence.requests.length}:failures=${evidence.requests.filter((request) => (request.status ?? 0) >= 400).length}`,
            )
            ...
            const scanSecrets = (await import('node:fs/promises'))
              .readFile(descriptor.scan_secrets_path, 'utf8')
            ...
            failure = error
            if (context && page && evidence && !signal.aborted) {
            ...
            await runBrowserCleanupIfActive(
              signal,
              () => progress('cleanup-deferred-to-outer-owned-root:deadline-aborted'),
              async (mayContinueCleanup) => {
                if (page) {
                  for (const [name, operation] of [
                    ...
                    if (!mayContinueCleanup()) return
                    const result = await action(page, csrfToken, 'gateway', operation, { name }).catch((error) => ({
                      status: 0,
                      body: String(error),
                    }))
                    ...
                    if (![200, 404].includes(result.status))
                      cleanupFailures.push(`${operation}(${name})=${result.status}`)
                  }
                }
                if (!mayContinueCleanup()) return
                if (context && !failure) await context.close()
                else await context?.close().catch(() => undefined)
                if (!failure)
                  assert.deepEqual(cleanupFailures, [], `live browser cleanup failed: ${cleanupFailures.join(', ')}`)
              },
            )
          }
        },
...
test(
  'nightly mobile viewport has no overflow and essential landmarks',
  {
    concurrency: false,
      ...
      await useBrowserWithAbort(
        ...
        async (browser) => {
          ...
          try {
            ...
            await runBrowserCleanupIfActive(
              signal,
              () => progress('nightly-cleanup-deferred-to-outer-owned-root:deadline-aborted'),
              async () => context?.close().catch(() => undefined),
            )
          }
        },
      )
    }, 'nightly mobile Gateway Admin journey')
  },
)
