  ...
  const publish = async (): Promise<PublishAttempt> => {
    ...
    try {
      ...
      if (submittingSessionEpoch !== getBrowserSessionEpoch()) {
        return { ok: false, error: 'The session changed while publishing. Check Depot jobs before retrying.' }
      }
      ...
      const message = error instanceof Error ? error.message : 'Publishing failed. Check Depot jobs before retrying.'
