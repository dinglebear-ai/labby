...
| `pool/catalog_publication.rs` | Immutable publication of the routable tool, regular-resource, regular-resource-template, and regular-prompt projections; deliberately excludes UI resources/templates, skills, OAuth subjects, local and synthetic rows, and unrelated capability health. Also owns each upstream's retained resource snapshot (`ResourceSource`, regular and `ui://` rows validated and byte-budgeted together, stamped with `listed_at`), the cache-only listing `cached_upstream_resources_allowed`, and `cold_resource_snapshots`, which classifies snapshots as missing or stale for the warm-up. A rejected snapshot (duplicate or malformed URI, oversized row, fleet byte-budget breach) is a per-upstream verdict: it withholds only that upstream from the projection and is recorded as its resources-capability error; it never unpublishes the fleet. An incarnation mismatch between a source and its entry remains a fleet-wide invariant `Err`. Each projection carries an independent generation so one catalog family cannot perturb another's publication identity. Owns the `CatalogWriteGuard` republish invariant: `catalog_write()` hands out the guard and its `Drop` calls `publish_if_changed()`, so no caller has to mark a projection dirty by hand. A `land:` merge silently dropped this module's `mod` declaration, which is what motivated the `xtask` module-reachability gate. |
